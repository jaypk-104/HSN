...
Ext.define('S_RET_TOT_HSPF.controller.MyGridController', {
    ...
    gridRetBtnClick : function(button, e, eOpts) {
		...
		for(var i=0; i<gridRecord.length; i++) {
			if(gridRecord[i].data['RET_DN_DT'] == null || gridRecord[i].data['RET_DN_DT'] == '' || gridRecord[i].data['RET_DN_DT'] == undefined ) {
				flag = 'F';
				msg = '반품출고일자를 선택하세요.';
				break;
			} 
			if(gridRecord[i].data['RET_DN_QTY'] == 0 ) {
				flag = 'F';
				msg = '반품출고수량을 선택하세요.';
				break;
			} 
    ...
    myGridClick : function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		store1.load({
    		params: {
    			...
    			V_COMP_ID: parent.Ext.getCmp('GCOMP_ID').getValue(),
    			V_USR_ID: parent.Ext.getCmp('GUSER_ID').getValue(),
    			V_REF_DN_NO: gridRecord[0].data['DN_NO'],
    			V_REF_DN_SEQ: gridRecord[0].data['DN_SEQ'],
    		},
