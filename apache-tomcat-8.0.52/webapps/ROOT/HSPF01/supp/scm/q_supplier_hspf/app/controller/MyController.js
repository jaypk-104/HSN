...
Ext.define('Q_SUPPLIER_HSPF.controller.MyController', {
    extend: 'Ext.app.Controller',
    
    control: {
        "#W_FILE": {
        	afterrender : 'W_FILEClick',
        },
        "#fileDelBtn" : { /*파일삭제*/
    ...
    W_FILEClick: function(component, eOpts) {
    	component.inputEl.on('dblclick',function(){
    		var filePopup = Ext.create("Q_SUPPLIER_HSPF.view.MyWindow");
    		filePopup.show();
//    		console.log("clclclclcl");
    		var width = Ext.getBody().getViewSize().width - 200;
    	    var height = Ext.getBody().getViewSize().height - 200;
    	    filePopup.setSize(width, height);
    	    filePopup.center();
...
//				                	Ext.getCmp('fileAddBtn').setText('[ ' + records.length + ' ]');
    ...
    fileGridDblClick:  function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		var V_COMP_ID = gridRecord[0].data['COMP_ID'];
		var V_FILE_NM = gridRecord[0].data['FILE_NM'];
		var V_FILE_IN_SYSTEM_NM = gridRecord[0].data['FILE_IN_SYSTEM_NM'];
		V_COMP_ID = encodeURI(V_COMP_ID);
		V_FILE_NM = encodeURI(V_FILE_NM);
		V_FILE_IN_SYSTEM_NM = encodeURI(V_FILE_IN_SYSTEM_NM);
		var myWin=new Ext.Window(
				{
					title : '파일다운로드',
					html : '<iframe name="xxx" border =0 src="sql/SCM_DLVY_HSPF_FILE.jsp?V_TYPE=D&V_COMP_ID='+V_COMP_ID+'&V_FILE_NM='+V_FILE_NM+'&V_FILE_IN_SYSTEM_NM='+V_FILE_IN_SYSTEM_NM+'" width="1%" height="1%"></iframe>',
					width :500,
					height:500
				});
		myWin.show();
