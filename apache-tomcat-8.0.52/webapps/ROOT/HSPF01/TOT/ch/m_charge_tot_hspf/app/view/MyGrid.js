...
Ext.define('M_CHARGE_TOT_HSPF.view.MyGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.myGrid',

    requires: [
        'M_CHARGE_TOT_HSPF.view.MyGridViewModel',
        'Ext.view.Table',
    			...
    			listeners : {
    				afterrender : function(c) {
    					c.getEl().on('dblclick', function() {
    						var popup = Ext.create("Popup.view.WinBpRegnoPop");
    						popup.show();
    						
    						Ext.getCmp('fieldType').setValue('M_CHARGE_TOT_HSPF');
...
//        	},
            summaryRenderer: function(val, params, data, metaData) {
                ...
                for ( var i = 0 ; i < allRecord.length ; i ++){
                    if(allRecord[i].data['CHARGE_CD'] == 'SPC24'){
                        val = val - allRecord[i].data['LOC_AMT'];
                    }
                }
        ...
        },
        {
            ...
            listeners: {
            	edit: function(editor, context, eOpts) {
            		...
            		var PAY_TYPE = context.record.get('PAY_TYPE');
...
//            		console.log(context.column.dataIndex);
            		...
            		if (context.column.dataIndex == 'VAT_TYPE' && VAT_TYPE == 'A') { //부가세유형 - 일반세금계산서
            			//context.record.set('VAT_AMT', LOC_AMT * 0.1);//VAT_REF_AMT
            		...
            		else if (context.column.dataIndex == 'VAT_TYPE' && VAT_TYPE == 'I') { //부가세유형 - 수입부가세
            			//context.record.set('VAT_AMT', LOC_AMT * 0.1);
            			context.record.set('VAT_AMT', VAT_REF_AMT * 0.1);
            			context.record.set('VAT_RATE', 10);
            		}
        			...
        			else if (context.column.dataIndex == 'PAY_TYPE'  && PAY_TYPE == 'Z1' ) {
            			context.record.set('PAY_AMT', LOC_AMT + (LOC_AMT * 0.1));
    				}	
        			else if (context.column.dataIndex == 'PAY_TYPE'  && PAY_TYPE == 'DP' ) {
            			context.record.set('PAY_AMT', LOC_AMT + (LOC_AMT * 0.1));
    				}	
        			else if (PAY_TYPE == 'B' && context.column.dataIndex == 'LOC_AMT') {
        				context.record.set('VAT_AMT', LOC_AMT * 0.1);
        				context.record.set('VAT_RATE', 0);
            			//context.record.set('PAY_AMT', LOC_AMT + (LOC_AMT * 0.1));
    				} else {
...
//    					context.record.set('VAT_RATE', 0); //부가세율
            		...
            		if(context.column.dataIndex == 'M_BP_CD') { //지급처코드 = 세금계산서발행처코드
            			if(context.record.get('TAX_BP_CD') == '' || context.record.get('TAX_BP_CD') == undefined || context.record.get('TAX_BP_CD') == null){
            		...
            		if(context.column.dataIndex == 'PAY_TYPE') { //지급유형
            			if(context.record.get("PAY_TYPE") == 'NO' || context.record.get("PAY_TYPE") == 'Z1') { //미지급, 가불금대체
//            				context.record.set('PAY_AMT', '0'); //지급	금액0 
            		...
            		if(context.column.dataIndex == 'CHARGE_CD'){
            			if(context.value == 'SPC24'){
            				context.record.set('EXP_ITEM_FLG', false);
            				context.record.set('LOC_AMT', 0);
            			} else {
            				context.record.set('EXP_ITEM_FLG', true);
            				if(context.value == 'SPC01' || context.value == 'SPC26' || 
            						context.value == 'SPC41' || context.value == 'SPC80'){
            					context.record.set('PAY_TYPE', 'DP');
            				}
            				
            				if(context.value == 'SPC26'){
            					var LC_DOC_NO = context.record.get('LC_DOC_NO');
            					
            					if(LC_DOC_NO.substr(0,5) == 'M3272'){ // 부산은행
            						context.record.set('BANK_CD', '32'); 
            						context.record.set('BANK_ACCT', '168-01-000130-9'); 
            					} else if(LC_DOC_NO.substr(0,5) == 'M3926'){ // 경남은행
            						context.record.set('BANK_CD', '39'); 
            						context.record.set('BANK_ACCT', '588-07-0017311');
            					} else if(LC_DOC_NO.substr(0,5) == 'M03QZ'){ // 농협
            						context.record.set('BANK_CD', '10'); 
            						context.record.set('BANK_ACCT', '317-0000-2230-41'); 
            					}
            				}
            			}
            	...
            	beforeedit: function (e, editor) {
					...
					if(field == 'LOC_AMT' && rec.CHARGE_CD == 'SPC24'){
						return false;
					}
