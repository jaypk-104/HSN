...
Ext.define('DUTY_CHARGE_OUT_DISTR_SELECT_GYEONGGI.controller.TbButtonController', {
    ...
    selBtnClick: function(button, e, eOpts) {
        ...
        store.load({
            params: {
                ...
                V_TAX_FROM_DT : Ext.getCmp('V_TAX_FROM_DT').getValue(),
                V_TAX_TO_DT : Ext.getCmp('V_TAX_TO_DT').getValue(),
            ...
            callback: function(records,operations,success){
                ...
                for(var i = 0 ; i < allRecord.length ; i ++){
                    totalAmount += Number(allRecord[i].data['TOTAL_AMOUNT']);
                    if(allRecord[i].data['PAY_YN'] == 'Y'){
                        payAmount += Number(allRecord[i].data['TOTAL_AMOUNT']);
                    }
                    else{
                        unpayAmount += Number(allRecord[i].data['TOTAL_AMOUNT']);
                    }
                }
    ...
    delBtnClick: function(button, e, eOpts) {
        ...
        if(gridRecord.length >= 1){
            for(var i = 0 ; i < gridRecord.length ; i ++){
                ...
                if(gridRecord[i].data['ERP_YN'] == 'Y'){
                    flag = 1;
                }
                if(gridRecord[i].data['GL_YN'] == 'Y'){
                    flag = 1;
                }
                if(gridRecord[i].data['HSPF_YN'] == 'Y'){
                    flag = 1;
                }
            }
            if(flag == 1){
                Ext.Msg.alert('확인', '삭제 불가능합니다. 관리자에게 문의해주세요.');
            }
            else{
