...
<%
	...
	try {
		...
		String V_SWM_YN = request.getParameter("V_SWM_YN") == null ? "" : request.getParameter("V_SWM_YN").toUpperCase();

		//상단
		if (V_TYPE.equals("S")) {

			cs = conn.prepareCall("call USP_003_M_IV_GR_REF_TOT_HSPF(?,?,?,?,?,?,?,?,?)");
			...
			cs.setString(9, V_SWM_YN);//V_USR_ID
			cs.executeQuery();
...
// 				subObject.put("VAT_AMT", rs.getString("VAT_AMT"));
				...
				subObject.put("SWM_YN", rs.getString("SWM_YN"));
			...
			cs = conn.prepareCall("call USP_003_M_IV_H_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
			...
			cs = conn.prepareCall("call USP_003_M_IV_D_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
// 			System.out.println("V_TYPE" + V_TYPE);
			...
			cs.setString(17, V_USR_ID);//V_USR_ID
			cs.registerOutParameter(18, com.tmax.tibero.TbTypes.CURSOR);
			cs.setString(19, V_REMARK);//V_REMARK
			...
			cs = conn.prepareCall("call USP_003_A_TEMP_IV_FR_TOT(?,?,?,?,?)");
