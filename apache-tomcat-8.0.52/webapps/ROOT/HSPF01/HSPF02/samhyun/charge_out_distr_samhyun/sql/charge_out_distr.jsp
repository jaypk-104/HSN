...
// 			String sql = "select B.CHARGE_TYPE, B.CHARGE_NAME , case when B.CHARGE_TYPE = 'SPC18' then 'V07' else null end VAT_CD 	from M_BP_CHARGE_H_INFO A (nolock)	LEFT OUTER jOIN M_BP_CHARGE_D_INFO B (nolock) ON A.CHARGE_TYPE = B.CHARGE_TYPE	where A.MAST_CD = 'CHARGE_OUT_DISTR_SAMHYUN' order by B.CHARGE_TYPE";
...
// 			rs = e_cs.executeQuery();
		...
		// 저장
			...
			cs.setString(4, "02360"); // 삼현만 조정하니까
			cs.setString(5, V_VESSEL_NM); //
			cs.setString(6, V_ITEM_NM); //
			cs.setString(7, null); //
			cs.setString(8, V_IN_DT); //
			cs.setInt(9, 0); //
			cs.setString(10,null); //
