...
// 		System.out.println("W_RISK_DT_FR" + W_RISK_DT_FR);
// 		System.out.println("W_RISK_DT_TO" + W_RISK_DT_TO);
...
// 				System.out.println("W_APPROV_NO" + W_APPROV_NO);
				...
				if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
					...
					for (int i = 0; i < jsonAr.size(); i++) {
						...
						cs = conn.prepareCall("call DISTR_E_APROV_DISTR.USP_E_APPROV_W_CALC(?,?,?)");
				...
				if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
					...
					for (int i = 0; i < jsonAr.size(); i++) {
						...
						if (V_COGS_CD.equals("M06")) {
							// 							System.out.println("V_TYPE" + V_TYPE);
							// 							System.out.println("V_COGS_CD" + V_COGS_CD);
							// 							System.out.println("V_COGS_AMT" + V_COGS_AMT);
							// 							System.out.println("V_APPROV_MGM_SEQ" + V_APPROV_MGM_SEQ);
						}
				...
				cs = conn.prepareCall("call DISTR_E_APROV_DISTR.USP_E_APPROV_MEAT_POP(?,?,?)");
				...
				while (rs.next()) {
					subObject = new JSONObject();
					subObject.put("NUM", rs.getString("NUM"));
					subObject.put("BF_TYPE", rs.getString("BF_TYPE"));
		...
		System.out.println("AA" + E_NO);
		System.out.println("BB" + E_SEQ);
