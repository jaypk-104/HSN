...
Ext.define('M_GR_TO_DN_TOT_HSPF.controller.MyGridController1', {
    ...
    control: {
        ...
        "#printBillRcptSumBtn": { /* 거래명세서 출력*/
        	click: 'printBillRcptSumBtnClick'
        },
		"#w_selBtn" : {
			click : 'w_selBtnClick'
		},
    ...
    gridDelBtn1Click: function(button, e, eOpts) {
    	...
    	for(var i=0; i<gridRecord1.length; i++) {
    		...
    		else if(gridRecord1[i].get('BARCODE_YN')  == 'Y') {
    			flag = 'F';
    			msg = '바코드 출고 데이터는 삭제 불가능합니다.';
    			break;
    		}
    		else {
    			flag = 'T';
...
//    		
//    		if(REQ_QTY > 0) {
//    			var popup = Ext.create("M_GR_TO_DN_TOT_HSPF.view.MyWindow1");
//
//        		var width = 1100;
...
//        		    			store.reload();
	...
	chargeUpdateBtnClick : function(button, e, eOpts) {
		...
		var popup = Ext.create("M_GR_TO_DN_TOT_HSPF.view.MyWindow2");
...
//		    	console.log(jsonResult);
	...
	gridSaveBtnClick : function(button, e, eOpts) {
		var myStore = Ext.getStore('MyStore1');

		for(var i=0; i<myStore.data.items.length; i++) {
			if(myStore.data.items[i].dirty) {
				myStore.data.items[i].set('V_TYPE', 'U');
			}
		}
	...
	printBillRcptBtnClick: function() {
    	...
    	if(Ext.getCmp('V_S_BP_CD2').getValue() != '') {
    		var BP_CD = Ext.getCmp('V_S_BP_CD2').getValue();
    		...
    		var params = 'BP_CD=' + BP_CD + '&DN_DTFR=' + DN_DTFR + '&DN_DTTO=' + DN_DTTO + '&V_DN_NO=' + V_DN_NO;
    		...
    		Ext.Msg.alert('확인', '매출처를 입력하세요.');
    	}
    },
    /* 거래명세서 출력*/
    printBillRcptSumBtnClick: function() {
    	var url = '';
    	
    	if(parent.Ext.getCmp('MAIN_SERVER_YN').getValue() == 'Y'){
    		var url_QTY = 'http://123.142.124.170:8080/aireport/on_server/S_DN_BILL_RECEIPT_SUM_TOT_QTY.jsp?';
    		var url_PRC = 'http://123.142.124.170:8080/aireport/on_server/S_DN_BILL_RECEIPT_SUM_TOT_PRC.jsp?';  	
    	} else{
    		var url_QTY = 'http://123.142.124.170:8080/aireport/on_server/S_DN_BILL_RECEIPT_TOT_QTY_TEST.jsp?';
    		var url_PRC = 'http://123.142.124.170:8080/aireport/on_server/S_DN_BILL_RECEIPT_TOT_PRC_TEST.jsp?';  	
    	}
