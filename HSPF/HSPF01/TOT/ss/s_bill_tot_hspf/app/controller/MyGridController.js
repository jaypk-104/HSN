...
Ext.define('S_BILL_TOT_HSPF.controller.MyGridController', {
	...
	cfmBtnClick : function(button, e, eOpts) {
		...
		if (Ext.getCmp('V_S_BP_NM2').getValue() == '' || Ext.getCmp('V_S_BP_NM2').getValue() == null) {
			...
			flag = 'F';
			msg = '매출일자를 입력하세요.';
		} else if ((!!!Ext.getCmp('V_PAY_DUR').getValue() || Ext.getCmp('V_PAY_DUR').getValue() < 0) && Ext.getCmp('V_SALE_TYPE').getValue() != 'XSO' && !!!Ext.getCmp('V_BILL_NO').getValue()) {
			flag = 'F';
			msg = '거래처 결제기준정보가 없습니다.';
			...
			flag = 'F';
			msg = '결제방법을 선택하세요.';
		} else if (Ext.getCmp('V_TO_SALES_GRP').getValue() == null) {
			flag = 'F';
			msg = '수금그룹을 선택하세요.';
		} else if (Ext.getCmp('V_COST_CD').getValue() == null) {
			flag = 'F';
			msg = '코스트센터를 선택하세요.';
		} else if (Ext.getCmp('V_VAT_TYPE').getValue() == null) {
			flag = 'F';
			msg = '부가세유형을 선택하세요.';
		} else if (Ext.getCmp('V_SALE_TYPE').getValue() == 'LSO' && Ext.getCmp('V_REMARK').getValue() == '') {
			flag = 'F';
			msg = '비고를 입력하세요.';
		} else {
			flag = 'T';
		...
		if (flag == 'T') {
			var dateDiff = Ext.Date.diff(Ext.getCmp('V_BILL_DT').getValue(),Ext.getCmp('V_DN_ISSUE_DT').getValue(),Ext.Date.DAY);
			
			var msg = '';
			if(dateDiff > 60){
				msg += '(수금만기일이 60일을 초과합니다)<br>'
			}
			msg += '매출채권등록 하시겠습니까?'
					...
					if (Ext.getCmp('V_BILL_NO').getValue() == '') {
						Ext.Ajax.request({
							...
							params : {
								...
								V_TO_SALES_GRP : Ext.getCmp('V_TO_SALES_GRP').getValue(),
