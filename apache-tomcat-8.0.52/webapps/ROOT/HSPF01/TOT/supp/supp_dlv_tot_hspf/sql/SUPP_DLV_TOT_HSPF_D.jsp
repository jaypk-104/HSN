...
<%
	...
	try {
		...
		//ASN팝업[상단HEADER 조회]
		if (V_TYPE.equals("HS")) {
			cs = conn.prepareCall("call USP_003_SUPP_DLV_TOT_HSPF_D(?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		//ASN팝업[하단DETAIL 조회]
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (i == jsonAr.size() - 1) {
						cs1 = conn.prepareCall("call USP_003_SUPP_DLV_TOT_HSPF_D(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call USP_003_SCM_BARCD_DIVISION_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_TYPE);
			cs.setString(2, V_COMP_ID);
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					//입고번호채번
					//M_GR_TOT_HSPF 에 어떻게 저장해야할지 몰라서 강제 저장, 출고는 잠시 보류
					/*
