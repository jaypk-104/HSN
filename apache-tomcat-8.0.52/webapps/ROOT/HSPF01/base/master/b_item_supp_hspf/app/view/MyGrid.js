	...
	plugins : [ {
		...
		listeners : {
			...
			afteredit: function(editor, context, eOpts) {
				if(context.column.dataIndex  == 'PO_TYPE'){
					var store = Ext.getStore('store1');
					var BPrecord = store.findRecord('DTL_CD',context.value,0, false, false, true);
					context.record.set('PO_TYPE_NM', BPrecord.data['DTL_NM']);
				}
				else if(context.column.dataIndex  == 'PAY_METH'){
					var store = Ext.getStore('store2');
					var BPrecord = store.findRecord('DTL_CD',context.value,0, false, false, true);
					context.record.set('PAY_METH_NM', BPrecord.data['DTL_NM']);
				}
				else if(context.column.dataIndex  == 'IN_TERMS'){
					var store = Ext.getStore('store3');
					var BPrecord = store.findRecord('DTL_CD',context.value,0, false, false, true);
					context.record.set('IN_TERMS_NM', BPrecord.data['DTL_NM']);
				}
				else if(context.column.dataIndex  == 'CUR'){
					var store = Ext.getStore('store4');
					var BPrecord = store.findRecord('DTL_CD',context.value,0, false, false, true);
					context.record.set('CUR_NM', BPrecord.data['DTL_NM']);
				}
				else if(context.column.dataIndex  == 'VAT_TYPE'){
					var store = Ext.getStore('store5');
					var BPrecord = store.findRecord('DTL_CD',context.value,0, false, false, true);
					context.record.set('VAT_TYPE_NM', BPrecord.data['DTL_NM']);
				}
			}
		}
	},
