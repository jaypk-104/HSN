...
<%
	...
	try {
		...
		if (V_TYPE.equals("S")) {
			...
			cs = conn.prepareCall("call USP_M_PO_UPLOAD_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			while (rs.next()) {
				...
				subObject.put("GR_SL_NM", rs.getString("GR_SL_NM"));
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					cs.setString(26, V_USR_ID);//V_USR_ID
					cs.registerOutParameter(27, com.tmax.tibero.TbTypes.CURSOR);
					cs.setString(28, V_GR_SL_CD);//V_GR_SL_CD
					cs.executeQuery();
					...
					// 					anySubObject.put("V_USR_ID", V_USR_ID);
					// 					anySubObject.put("V_GR_SL_CD", V_GR_SL_CD);
