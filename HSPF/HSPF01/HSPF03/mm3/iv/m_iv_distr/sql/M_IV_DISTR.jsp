...
<%
	...
	try {
		...
		if (V_TYPE.equals("S")) {
			...
			cs = conn.prepareCall("call DISTR_IV.USP_M_GR_REF_IV(?,?,?,?,?)");
			...
			cs = conn.prepareCall("call DISTR_IV.USP_M_IV_H_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
			...
			cs = conn.prepareCall("call DISTR_IV.USP_M_IV_D_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
// 			System.out.println("V_TYPE" + V_TYPE);
			...
			cs = conn.prepareCall("call DISTR_TEMP_GL.USP_A_TEMP_IV_FR(?,?,?,?,?)");
