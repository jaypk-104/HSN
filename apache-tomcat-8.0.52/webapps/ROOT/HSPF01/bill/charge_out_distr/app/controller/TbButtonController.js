...
Ext.define('M_CHARGE_OUT_DISTR2.controller.TbButtonController', {
        ...
        if(Ext.getCmp('V_M_CHARGE_NO').getValue() == '' || Ext.getCmp('V_M_CHARGE_NO').getValue() == null || Ext.getCmp('V_M_CHARGE_NO').getValue() == undefined){
            ...
            Ext.Ajax.request({
                ...
                success : function(response) {
                    ...
                    Ext.getCmp('V_BASE_DT').setValue(resultList.BASE_DT);
                    Ext.getCmp('V_BL_DOC_NO').setValue(resultList.BL_DOC_NO);
                    Ext.getCmp('V_RK_AMT').setValue(resultList.RK_AMT);
                    Ext.getCmp('V_ERP_YN').setValue(resultList.ERP_YN);
    ...
    saveBtnClick: function(button, e, eOpts) {
        ...
        if(Ext.getCmp('V_ERP_YN').getValue() == 'Y'){
            Ext.Msg.alert('확인','이미 승인된 항목입니다.<br>담당자에게 문의해주세요.');
        }
        else{
            ...
            if(Ext.getCmp('V_BASE_DT').getValue() == '' || Ext.getCmp('V_BASE_DT').getValue() == null || Ext.getCmp('V_BASE_DT').getValue() == undefined){
                flag = 1;
                msg = '기준일을 입력해주세요.';
                Ext.getCmp('V_BASE_DT').focus();
            }
            else if (Ext.getCmp('V_BASE_DT').wasValid == false){
                flag = 1;
                msg = '기준일을 다음과 같은 형식으로 입력해주세요.<br> 2019-01-01';
                Ext.getCmp('V_BASE_DT').focus();
            }
            else if(Ext.getCmp('V_BL_DOC_NO').getValue() == '' || Ext.getCmp('V_BL_DOC_NO').getValue() == null || Ext.getCmp('V_BL_DOC_NO').getValue() == undefined){
                flag = 1;
                msg = 'BL번호를 입력해주세요.';
                Ext.getCmp('V_BL_DOC_NO').focus();
            }
            else if(fileStore.getRange().length == 0 && parent.Ext.getCmp('GBP_CD').getValue() != '00000'){
                flag = 1;
                msg = '첨부파일을 넣어주세요.';
            }
            /*
            else if(Ext.getCmp('V_RK_AMT').getValue() == '' || Ext.getCmp('V_RK_AMT').getValue() == null || Ext.getCmp('V_RK_AMT').getValue() == undefined){
                flag = 1;
                msg = '미수금을 입력해주세요.';
                Ext.getCmp('V_RK_AMT').focus();
            }
            */
            ...
            for(var i = 0 ; i < fileRecord.length ; i ++){
                if(fileRecord[i].phantom == true){ // phantom이 true인 것(새로 추가된것) 만 저장되도록.
                                ...
                                success : function(response) {
                                    ...
                                    Ext.Ajax.request({
                                        ...
                                        params: {
                                            ...
                                            V_M_CHARGE_NO: Ext.getCmp('V_M_CHARGE_NO').getValue(),
                                            V_BASE_DT: Ext.getCmp('V_BASE_DT').getValue(),
                                            V_BL_DOC_NO: Ext.getCmp('V_BL_DOC_NO').getValue(),

                                            V_RK_AMT: Ext.getCmp('V_RK_AMT').getValue(),
        ...
        Ext.MessageBox.confirm('확인', '저장하지 않은 내용은 사라집니다.<br>새문서로 이동 하시겠습니까?', function(btn) {
            if(btn == 'yes') {
                ...
                var dynamicTab={
                    ...
                    title : '부대경비정산(조광)',
                    autoScroll: true,
                    html : '<iframe name="xxx" border =0 src="/HSPF01/bill/charge_out_distr/CHARGE_OUT_DISTR.jsp" width="100%" height="100%"></iframe>',
                    closable : true
                };
