    	...
    	if(flag == 'T') {
    		store.sync({
    			...
    			success : function(response) {
    				var myResponse = response.operations["0"]._response.responseText;
    				var jsonResult = Ext.JSON.decode(myResponse);
    				
    				var resMSG = jsonResult.resMSG;
    				var finCnt = jsonResult.finCnt;
    				var resDATE = jsonResult.resDATE;
    				var resString = jsonResult.resString;
    				var resPoDesc = jsonResult.resPoDesc;
    				
    				if(resMSG == 'ERROR') {
    					Ext.Msg.alert('확인', '발주업로드 오류(관리자 문의요망)<br>' + resString + '<br>' + resDATE);
