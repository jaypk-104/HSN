...
Ext.define('S_BILL_TOT_HSPF.view.MyGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.myGrid',

    requires: [
        'S_BILL_TOT_HSPF.view.MyGridViewModel',
...
//				V_EXPECT_LOC_AMT += Math.round(record.get('DN_AMT') * XCH_RATE);
//				Ext.getCmp('V_EXPECT_LOC_AMT').setValue(Ext.util.Format.number(V_EXPECT_LOC_AMT, '0,000'));
				...
				if(store1.getCount() > 0) {
					if (Ext.getCmp('V_S_BP_CD2').getValue() != record.get('BP_CD')) {
						...
						store1.insert(store1Cnt, [ ({
							...
							BILL_AMT : (record.get('DN_QTY') * record.get('DN_PRC')).toFixed(4),
							BILL_LOC_AMT : Math.round((record.get('DN_QTY') * record.get('DN_PRC')).toFixed(4) * XCH_RATE),
...
//							DN_PRC : record.get('DN_PRC'),
							...
							VAT_AMT : Math.floor(Math.round((record.get('DN_QTY') * record.get('DN_PRC')).toFixed(4) * XCH_RATE) * record.get('VAT_RATE') / 100),
							VAT_RATE : record.get('VAT_RATE'),
							DN_ISSUE_DT : record.get('DN_ISSUE_DT'),
							COST_CD : Ext.getCmp('V_COST_CD').getValue(),
						}) ]);
...
//						Ext.getCmp('V_IN_TERMS').setValue(record.get('IN_TERMS'));
					...
					Ext.getCmp('V_VAT_TYPE').setValue(record.get('VAT_TYPE'));
...
//						DN_PRC : record.get('DN_PRC'),
					...
					Ext.getCmp('V_TO_SALES_GRP').setValue(record.get('TO_SALES_GRP'));
					...
					var nows = new Date();
					nows.setDate(nows.getDate() + Number(record.get('PAY_DUR')));
