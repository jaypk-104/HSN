...
<%
	...
	try {
		...
		//관리구분팝업조회
		if (V_TYPE.equals("SP")) {
			cs = conn.prepareCall("call DISTR_CG2.USP_M_CHARGE_POPUP_DISTR(?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call DISTR_CG2.USP_M_CHARGE_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (V_TYPE.equals("I")) {
						...
						cs = conn.prepareCall("call DISTR_TEMP_GL_CG2.USP_TEMP_M_CHARGE_NO(?,?)");
						...
						/*제일 마지막 한번*/
						if (i == jsonAr.size() - 1) {
							cs = conn.prepareCall("call DISTR_TEMP_GL_CG2.USP_A_TEMP_CH_FR(?,?,?,?,?,?,?,?)");
							cs.setString(1, V_COMP_ID);//V_COMP_ID
