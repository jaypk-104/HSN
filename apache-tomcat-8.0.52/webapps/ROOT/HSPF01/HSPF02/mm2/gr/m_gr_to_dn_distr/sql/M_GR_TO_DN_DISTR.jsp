...
<%
	...
	try {
		...
		String V_LC_DOC_NO = request.getParameter("V_LC_DOC_NO") == null ? ""
				: request.getParameter("V_LC_DOC_NO").toUpperCase();
		...
		String V_MVMT_NO = request.getParameter("V_MVMT_NO") == null ? ""
				: request.getParameter("V_MVMT_NO").toUpperCase();
		String V_DN_DT_FR = request.getParameter("V_DN_DT_FR") == null ? ""
				: request.getParameter("V_DN_DT_FR").toUpperCase().substring(0, 10);
		String V_DN_DT_TO = request.getParameter("V_DN_DT_TO") == null ? ""
				: request.getParameter("V_DN_DT_TO").toUpperCase().substring(0, 10);
		String V_DN_DT = request.getParameter("V_DN_DT") == null ? ""
				: request.getParameter("V_DN_DT").toUpperCase().substring(0, 10);
		String V_S_BP_NM = request.getParameter("V_S_BP_NM") == null ? ""
				: request.getParameter("V_S_BP_NM").toUpperCase();

		//상단
		if (V_TYPE.equals("SH")) {

			cs = conn.prepareCall(
					"call DISTR_MD.USP_M_GR_TO_DN_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			sql += "       DN_BOX_QTY, CASE WHEN NVL(A.DN_QTY,0) = 0 THEN 0 ELSE ROUND((DN_LOC_AMT + ADD_DN_AMT)/A.DN_QTY,2) END DN_PRC,";
			sql +="		DN_LOC_AMT + ADD_DN_AMT AS DN_LOC_AMT, COG_AMT, COG_FORWAD_AMT, A.SL_AMT,                                                              ";
			sql += "       A.FORD_AMT, A.INTER_AMT, (NVL(A.SL_AMT,0) + NVL(A.FORD_AMT,0) + NVL(A.INTER_AMT,0)) SALE_COGS_SUM_AMT, MARGIN_RATE, MARGIN_AMT,";
			sql += "       A.MVMT_NO, NVL(E.BL_DOC_NO, I.BL_DOC_NO) BL_DOC_NO, CASE WHEN NVL(A.DN_QTY,0) = 0 THEN 0 ELSE NVL(ROUND(A.COG_AMT/A.DN_QTY, 2), 0) END SALE_PRC,A.DN_REAL_QTY                                                           ";
			...
			sql += "       NVL(A.DN_DT-(CASE WHEN I.MVMT_NO IS NOT NULL THEN H.MVMT_DT ELSE K.IN_DT END)+1, 0) OWN_DT, TO_CHAR(A.SALE_ISSUE_DT, 'YYYY-MM-DD') SALE_ISSUE_DT ";
			...
			sql += "                       LEFT OUTER JOIN M_CC_DTL_DISTR E2                                        ";
	        sql += "                       ON     D.CC_NO = E2.CC_NO                                                ";
	        sql += "                       AND    D.CC_SEQ = E2.CC_SEQ                                              ";
			...
			sql += " LEFT OUTER JOIN M_GR_KOR_INFO_DISTR I ";
			sql += " ON A.MVMT_NO = I.MVMT_NO ";
			sql += " LEFT OUTER JOIN M_CY_GR_D_DISTR J  ";
			sql += " ON E2.BL_NO = J.BL_NO AND E2.BL_SEQ = J.BL_SEQ ";
			sql += " LEFT OUTER JOIN M_CY_GR_H_DISTR K  ";
			sql += " ON J.CY_NO = K.CY_NO ";

			//전체보기
			if (V_BL_DOC_NO.equals("")) {
				sql += " WHERE A.MVMT_NO IN " + V_MVMT_NO + "";
			} else if (!V_BL_DOC_NO.equals("*")) {
				sql += " WHERE      NVL(E.BL_DOC_NO,I.BL_DOC_NO) LIKE '%" + V_BL_DOC_NO
						+ "%'                                                                               ";
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					String V_DN_NO = request.getParameter("V_DN_NO") == null ? ""
							: request.getParameter("V_DN_NO").toUpperCase();
					V_DN_DT = request.getParameter("V_DN_DT") == null ? ""
							: request.getParameter("V_DN_DT").toUpperCase().substring(0, 10);
					...
					if (!V_TYPE.equals("")) {
						...
						String V_S_BP_CD = hashMap.get("S_BP_CD") == null ? ""
								: hashMap.get("S_BP_CD").toString();
						String V_SALE_TYPE = hashMap.get("SALE_TYPE") == null ? ""
								: hashMap.get("SALE_TYPE").toString();
						String V_DN_CUR = hashMap.get("DN_CUR") == null ? "" : hashMap.get("DN_CUR").toString();
						String V_DN_XCHG_RT = hashMap.get("DN_XCHG_RT") == null ? ""
								: hashMap.get("DN_XCHG_RT").toString();
						String V_ITEM_CD = hashMap.get("ITEM_CD") == null ? ""
								: hashMap.get("ITEM_CD").toString();
						String V_DN_QTY = hashMap.get("DN_QTY") == null ? "" : hashMap.get("DN_QTY").toString();
						String V_REQ_QTY = hashMap.get("REQ_QTY") == null ? ""
								: hashMap.get("REQ_QTY").toString();
						...
						String V_DN_BOX_QTY = hashMap.get("REQ_BOX_QTY") == null ? ""
								: hashMap.get("REQ_BOX_QTY").toString();
						String V_DN_BOX_WGT_UNIT = hashMap.get("DN_BOX_WGT_UNIT") == null ? ""
								: hashMap.get("DN_BOX_WGT_UNIT").toString();
						String V_DN_REAL_QTY = hashMap.get("DN_REAL_QTY") == null ? ""
								: hashMap.get("DN_REAL_QTY").toString();
						...
						String V_DN_LOC_AMT = hashMap.get("DN_LOC_AMT") == null ? ""
								: hashMap.get("DN_LOC_AMT").toString();
						String V_SALE_ISSUE_DT = hashMap.get("ISSUE_DT") == null ? ""
								: hashMap.get("ISSUE_DT").toString().substring(0, 10);
						...
						String V_DN_FINAL_AMT = hashMap.get("DN_TOTAL_AMT") == null ? ""
								: hashMap.get("DN_FINAL_AMT").toString();

						String V_ADD_DN_AMT = hashMap.get("ADD_DN_AMT") == null ? ""
								: hashMap.get("ADD_DN_AMT").toString();
						...
						if (!V_TYPE.equals("IH") && !V_TYPE.equals("ID")) {
							...
							V_DN_DT = hashMap.get("DN_DT") == null ? ""
									: hashMap.get("DN_DT").toString().substring(0, 10);
						}
						String V_DN_ISSUE_DT = request.getParameter("V_DN_ISSUE_DT") == null ? ""
								: request.getParameter("V_DN_ISSUE_DT").toUpperCase().substring(0, 10);
				...
				if (!V_TYPE.equals("")) {
					...
					String V_S_BP_CD = jsondata.get("S_BP_CD") == null ? ""
							: jsondata.get("S_BP_CD").toString();
					String V_SALE_TYPE = jsondata.get("SALE_TYPE") == null ? ""
							: jsondata.get("SALE_TYPE").toString();
					String V_DN_CUR = jsondata.get("DN_CUR") == null ? "" : jsondata.get("DN_CUR").toString();
					String V_DN_XCHG_RT = jsondata.get("DN_XCHG_RT") == null ? ""
							: jsondata.get("DN_XCHG_RT").toString();
					String V_ITEM_CD = jsondata.get("ITEM_CD") == null ? ""
							: jsondata.get("ITEM_CD").toString();
					String V_DN_QTY = jsondata.get("DN_QTY") == null ? "" : jsondata.get("DN_QTY").toString();
					String V_REQ_QTY = jsondata.get("REQ_QTY") == null ? ""
							: jsondata.get("REQ_QTY").toString();
					...
					String V_DN_BOX_QTY = jsondata.get("REQ_BOX_QTY") == null ? ""
							: jsondata.get("REQ_BOX_QTY").toString();
					String V_DN_BOX_WGT_UNIT = jsondata.get("DN_BOX_WGT_UNIT") == null ? ""
							: jsondata.get("DN_BOX_WGT_UNIT").toString();
					String V_DN_REAL_QTY = jsondata.get("DN_REAL_QTY") == null ? ""
							: jsondata.get("DN_REAL_QTY").toString();
					...
					String V_DN_LOC_AMT = jsondata.get("DN_LOC_AMT") == null ? ""
							: jsondata.get("DN_LOC_AMT").toString();
					String V_SALE_ISSUE_DT = jsondata.get("ISSUE_DT") == null ? ""
							: jsondata.get("ISSUE_DT").toString().substring(0, 10);
					...
					String V_DN_FINAL_AMT = jsondata.get("DN_TOTAL_AMT") == null ? ""
							: jsondata.get("DN_FINAL_AMT").toString();
					String V_ADD_DN_AMT = jsondata.get("ADD_DN_AMT") == null ? ""
							: jsondata.get("ADD_DN_AMT").toString();
					String V_DN_NO = request.getParameter("V_DN_NO") == null ? ""
							: request.getParameter("V_DN_NO").toUpperCase();
					V_DN_DT = request.getParameter("V_DN_DT") == null ? ""
							: request.getParameter("V_DN_DT").toUpperCase().substring(0, 10);
					...
					if (!V_TYPE.equals("IH") && !V_TYPE.equals("ID")) {
						...
						V_DN_DT = jsondata.get("DN_DT") == null ? ""
								: jsondata.get("DN_DT").toString().substring(0, 10);
					}
			...
			String V_BS_DT = request.getParameter("V_BS_DT") == null ? ""
					: request.getParameter("V_BS_DT").substring(0, 10);
			String V_BS_BOX_QTY = request.getParameter("V_BS_BOX_QTY") == null ? ""
					: request.getParameter("V_BS_BOX_QTY");
			String V_BS_QTY = request.getParameter("V_BS_QTY") == null ? "" : request.getParameter("V_BS_QTY");
			String V_BAS_STEP = request.getParameter("V_BAS_STEP") == null ? ""
					: request.getParameter("V_BAS_STEP");
			...
			String V_ITEM_CD = request.getParameter("V_ITEM_CD") == null ? ""
					: request.getParameter("V_ITEM_CD");
