...
// 		String V_APPROV_NO = request.getParameter("V_APPROV_NO") == null ? "" : request.getParameter("V_APPROV_NO").toUpperCase();
		...
		if (V_TYPE.equals("SH")) {
			cs = conn.prepareCall("call USP_003_M_BL_PO_N_LC_REF_TOT_HSPF(?,?,?,?,?,?,?,?)");
			...
			cs.setString(8, V_PO_USR_NM);//V_PO_USR_NM
			cs.executeQuery();
...
// 				subObject.put("PLANT_NO", rs.getString("PLANT_NO"));
				...
				subObject.put("FIRST_YN", rs.getString("FIRST_YN"));
			...
			cs = conn.prepareCall("call USP_003_M_BL_H_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			cs.setString(22, "");//V_FIRST_YN
			cs.setString(23, "");//V_BL_TYPE
			cs.setString(24, "");//V_IV_DT
			cs.executeQuery();
			...
			String V_IV_DT = request.getParameter("V_IV_DT") == null || request.getParameter("V_IV_DT") == "" ? "" : request.getParameter("V_IV_DT").substring(0, 10);
			...
			String V_FIRST_YN = request.getParameter("V_FIRST_YN") == null ? "" : request.getParameter("V_FIRST_YN").toUpperCase();
			String V_BL_TYPE = request.getParameter("V_BL_TYPE") == null ? "" : request.getParameter("V_BL_TYPE").toUpperCase();
			
			if(V_FIRST_YN.equals("TRUE")) {
				V_FIRST_YN = "1";	
			} else {
				V_FIRST_YN = "";	
			}
			...
			cs.setString(22, V_FIRST_YN);//V_FIRST_YN
			cs.setString(23, V_BL_TYPE);//V_BL_TYPE
			cs.setString(24, V_IV_DT);//V_IV_DT
			cs.executeQuery();
...
// 						V_DLVY_HOPE_DT = "";
// 					} else if (V_DLVY_HOPE_DT == null || V_DLVY_HOPE_DT.equals("")) {
// 					} else {
