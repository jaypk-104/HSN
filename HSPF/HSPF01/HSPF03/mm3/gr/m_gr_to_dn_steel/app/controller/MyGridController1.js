...
Ext.define('M_GR_TO_DN_STEEL.controller.MyGridController1', {
    ...
    myGridDblClick1: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
    	...
    	if((cellIndex != 31)) {
    		...
    		if(REQ_QTY > 0) {
    			var popup = Ext.create("M_GR_TO_DN_STEEL.view.MyWindow1");
    			...
    			Ext.Ajax.request({
    				url : 'sql/M_GR_TO_DN_STEEL.jsp',
    				method : 'post',
    				params : {
    					V_TYPE : 'CALC_POP',
    					V_BS_DT: Ext.getCmp('V_DN_DT').getValue(),
    					V_BS_BON_QTY: record.get('REQ_BON_QTY'),
    					V_BS_QTY: REQ_QTY,
    					V_BAS_STEP : 'DP1',
...
//    					Ext.getCmp('W_MG').setValue(Ext.util.Format.number(jsonResult.resultList[0].MG_RT, '0,000.00') + ' / ' + Ext.util.Format.number(jsonResult.resultList[0].MG_AMT, '0,000') );
    					...
    					Ext.getCmp('W_SPOT_XCH_RT').setValue(Ext.util.Format.number(jsonResult.resultList[0].SPOT_XCH_RT, '0,000.00')); //선적환율
    					
    					Ext.getCmp('W_USANCE_AMT').setValue(Ext.util.Format.number(jsonResult.resultList[0].USANCE_AMT, '0,000')); //유산스이자
    				}
    			});
    		} else {
    			Ext.Msg.alert('확인', '출고요청 BON수량을 입력하세요.');
    		}
...
//    	if((cellIndex >= 10 && cellIndex <= 15) || (cellIndex >= 20 && cellIndex <= 23)) {
		if(cellIndex >= 11 && cellIndex <= 19 ) {
        	...
        	console.log('zzz'  + record.get('DN_SEQ'));
...
//        		    			store.reload();
	...
	chargeUpdateBtnClick : function(button, e, eOpts) {
		...
		var popup = Ext.create("M_GR_TO_DN_STEEL.view.MyWindow2");
		 ...
		 Ext.Ajax.request({
			    ...
			    success : function(response) {
			    	...
			    	var jsonResult = Ext.JSON.decode(response.responseText);
		 			...
		 			Ext.getCmp('W_BON_QTY').setValue(jsonResult.resultList[0].BON_QTY);
