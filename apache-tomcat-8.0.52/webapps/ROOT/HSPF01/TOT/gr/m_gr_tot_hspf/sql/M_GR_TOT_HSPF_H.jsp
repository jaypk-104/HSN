...
<%
	...
	try {
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call USP_003_M_GR_BL_N_CC_REF_TOT_HSPF(?,?,?,?,?,?,?,?)");
			...
			cs.setString(8, V_IV_TYPE);//V_IV_TYPE
			cs.executeQuery();
...
// 				subObject.put("BOX_WGT_UNIT", rs.getString("BOX_WGT_UNIT"));
			...
			V_IV_TYPE = request.getParameter("V_IV_TYPE") == null ? "" : request.getParameter("V_IV_TYPE").toUpperCase();
...
// 			cs = conn.prepareCall("call USP_003_M_GR_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
// 			cs.setString(1, V_COMP_ID);//V_COMP_ID
...
// 			cs.setString(29, V_MVMT_DT_FR);//V_MVMT_DT_FR
// 			cs.setString(30, V_MVMT_DT_TO);//V_MVMT_DT_TO
// 			cs.setString(31, V_BL_DOC_NO);//V_BL_DOC_NO
// 			cs.executeQuery();

// 			rs = (ResultSet) cs.getObject(27);

			String sql = "";
			sql = sql + " SELECT A.MVMT_NO, A.M_BP_CD, B.BP_NM M_BP_NM, A.IV_TYPE, D.DTL_NM IV_TYPE_NM,                 ";
			sql = sql + "        TO_CHAR(A.MVMT_DT, 'YYYY-MM-DD') MVMT_DT, I.LC_DOC_NO, G.BL_DOC_NO, '' PAY_TYPE_NM, A.ITEM_CD, C.ITEM_NM,             ";
			sql = sql + "        C.SPEC, A.CUR, C.UNIT, A.DOC_AMT, A.LOC_AMT, A.FORWARD_XCH_AMT, A.DISTR_AMT TOTAL_AMT, ";
			sql = sql + "        A.SL_CD, A.LOC_AMT AC_AMT, A.FORWARD_XCH_AMT SL_AMT, A.CC_NO, A.CC_SEQ, F.PRICE,       ";
			sql = sql + "        A.PO_NO, A.PO_SEQ, A.XCHG_RT, NVL(K.USR_NM,A.INSRT_USR_ID) GR_USR_NM, A.FORWARD_XCH_RT, ";
			sql = sql + "        A.GR_NO, A.TEMP_GL_NO , A.TEMP_PRC_YN, J.SL_NM, A.QTY,                                 ";
			sql = sql + "        O.LO_OPEN_AMT LC_OPEN_AMT, L.CC_AMT TONG_AMT, M.TAX_AMT GWAN_AMT,					    ";
			sql = sql + "        N.REC_AMT INSU_AMT, Q.ETC_AMT, P.INSUR_AMT, R.LO_OPEN_AMT LC_AMEND_AMT   				";
// 			sql = sql + "        ,CASE WHEN S.MVMT_NO IS NULL THEN 'N' ELSE 'Y' END DN_YN                                ";
			sql = sql + " FROM    M_GR_TOT_HSPF A                                                                       ";
			sql = sql + "        LEFT OUTER JOIN B_BIZ_HSPF B                                                           ";
			sql = sql + "        ON       A.COMP_ID=B.COMP_ID                                                           ";
			sql = sql + "        AND      A.M_BP_CD=B.BP_CD                                                             ";
			sql = sql + "        LEFT OUTER JOIN B_ITEM_HSPF C                                                          ";
			sql = sql + "        ON       A.COMP_ID=C.COMP_ID                                                           ";
			sql = sql + "        AND      A.ITEM_CD=C.ITEM_CD                                                           ";
			sql = sql + "        LEFT OUTER JOIN B_DTL_INFO D                                                           ";
			sql = sql + "        ON       A.COMP_ID=D.COMP_ID                                                           ";
			sql = sql + "        AND      A.IV_TYPE=D.DTL_CD                                                            ";
			sql = sql + "        AND      D.MAST_CD='MA02'                                                              ";
			sql = sql + "        LEFT OUTER JOIN M_CC_DTL_TOT_HSPF E                                                    ";
			sql = sql + "        ON       A.COMP_ID=E.COMP_ID                                                           ";
			sql = sql + "        AND      A.CC_NO  =E.CC_NO                                                             ";
			sql = sql + "        AND      A.CC_SEQ =E.CC_SEQ                                                            ";
			sql = sql + "        LEFT OUTER JOIN M_BL_DTL_TOT_HSPF F                                                    ";
			sql = sql + "        ON       E.COMP_ID=F.COMP_ID                                                           ";
			sql = sql + "        AND      E.BL_NO  =F.BL_NO                                                             ";
			sql = sql + "        AND      E.BL_SEQ =F.BL_SEQ                                                            ";				
			sql = sql + "        LEFT OUTER JOIN M_BL_HDR_TOT_HSPF G                                                    ";
			sql = sql + "        ON       F.COMP_ID=G.COMP_ID                                                           ";
			sql = sql + "        AND      F.BL_NO  =G.BL_NO                                                             ";
			sql = sql + "        LEFT OUTER JOIN M_LC_DTL_TOT_HSPF H                                                    ";
			sql = sql + "        ON       F.COMP_ID=H.COMP_ID                                                           ";
			sql = sql + "        AND      F.LC_NO  =H.LC_NO                                                             ";
			sql = sql + "        AND      F.LC_SEQ =H.LC_SEQ                                                            ";
			sql = sql + "        LEFT OUTER JOIN M_LC_HDR_TOT_HSPF I                                                    ";
			sql = sql + "        ON       H.COMP_ID=I.COMP_ID                                                           ";
			sql = sql + "        AND      H.LC_NO  =I.LC_NO                                                             ";
			sql = sql + "        LEFT OUTER JOIN B_STORAGE_HSPF J                                                       ";
			sql = sql + "        ON       A.COMP_ID=J.COMP_ID                                                           ";
			sql = sql + "        AND      A.SL_CD  =J.SL_CD                                                             ";
			sql = sql + "        LEFT OUTER JOIN Z_USR_INFO_HSPF K                                                      ";
			sql = sql + "        ON       A.INSRT_USR_ID = K.USR_ID                                                     ";

			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_CC L                                                ";
			sql = sql + "         ON       A.MVMT_NO = L.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_TAX M                                               ";
			sql = sql + "         ON       A.MVMT_NO = M.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_REC N                                               ";
			sql = sql + "         ON       A.MVMT_NO = N.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_LC_OPEN_ONLY O                                      ";
			sql = sql + "         ON       A.MVMT_NO = O.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_INS P                                               ";
			sql = sql + "         ON       A.MVMT_NO = P.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_ETC Q                                               ";
			sql = sql + "         ON       A.MVMT_NO = Q.MVMT_NO                                                        ";
			sql = sql + "         LEFT OUTER JOIN M_CHARGE_TOT_HSPF_LC_OPEN_AMEND R                                     ";
			sql = sql + "         ON 	  A.MVMT_NO=R.MVMT_NO                                                           ";

// 			sql = sql + "        LEFT OUTER JOIN M_GR_TO_DN_TOT_HSPF S                                                  ";
// 			sql = sql + "        ON       A.MVMT_NO = S.MVMT_NO                                                         ";

			sql += " WHERE    A.COMP_ID          = '" + V_COMP_ID + "'                                                  ";
			sql += " AND      A.IV_TYPE IN       ( 'PO', 'EXIV' )                                                 		";
			//전체보기
			if (V_BL_DOC_NO.equals("") && !V_IV_TYPE.equals("TO")) {
...
// 			sql = sql + "        ,CASE WHEN S.MVMT_NO IS NULL THEN 'N' ELSE 'Y' END DN_YN                                ";
			...
			sql = sql + "        AND      D.MAST_CD='MA02'                                                              ";
			sql = sql + "        LEFT OUTER JOIN M_BL_DTL_TOT_HSPF F                                                    ";
			sql = sql + "        ON       A.COMP_ID=F.COMP_ID                                                           ";
			sql = sql + "        AND      A.BL_NO  =F.BL_NO                                                             ";
			sql = sql + "        AND      A.BL_SEQ =F.BL_SEQ                                                            ";
			sql = sql + "        LEFT OUTER JOIN M_BL_HDR_TOT_HSPF G                                                    ";
			sql = sql + "        ON       F.COMP_ID=G.COMP_ID                                                           ";
			sql = sql + "        AND      F.BL_NO  =G.BL_NO                                                             ";
			sql = sql + "        LEFT OUTER JOIN M_LC_DTL_TOT_HSPF H                                                    ";
...
// 			sql = sql + "        ON       A.MVMT_NO = S.MVMT_NO                                                         ";
			...
			sql += " ORDER BY A.GR_NO, A.ITEM_CD;                                                                        ";
...
// 				subObject.put("DN_YN", rs.getString("DN_YN"));
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					String V_QTY = hashMap.get("REQ_QTY") == null ? "" : hashMap.get("REQ_QTY").toString();
					...
					V_IV_TYPE = hashMap.get("IV_TYPE") == null ? "" : hashMap.get("IV_TYPE").toString();
					...
					cs.setString(23, V_GR_SL_CD);//V_SL_CD
					cs.setString(24, V_CC_NO);//V_CC_NO
					cs.setString(25, V_CC_SEQ);//V_CC_SEQ
					cs.setString(26, V_USR_ID);//V_USR_ID
					cs.registerOutParameter(27, com.tmax.tibero.TbTypes.CURSOR);
					cs.setString(28, V_TEMP_PRC_YN);//V_TEMP_PRC_YN
					cs.setString(29, "");//V_MVMT_DT_FR
					cs.setString(30, "");//V_MVMT_DT_TO
					cs.setString(31, "");//V_BL_DOC_NO
					cs.setString(32, V_BL_NO);//V_BL_NO
					cs.setString(33, V_BL_SEQ);//V_BL_SEQ
					cs.executeQuery();
					...
					if (V_TYPE.equals("ID")) {
						...
						cs2 = conn.prepareCall("call USP_003_M_CHARGE_CALC_2(?,?,?)");
				...
				String V_QTY = jsondata.get("REQ_QTY") == null ? "" : jsondata.get("REQ_QTY").toString();
				...
				V_IV_TYPE = jsondata.get("IV_TYPE") == null ? "" : jsondata.get("IV_TYPE").toString();
			...
			cs.setString(30, "");//V_MVMT_DT_TO
			cs.setString(31, "");//V_BL_DOC_NO
			cs.setString(32, "");//V_BL_NO
			cs.setString(33, "");//V_BL_SEQ
			cs.executeQuery();
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					cs = conn.prepareCall("call USP_003_M_CHARGE_CALC_2(?,?,?)");
					cs.setString(1, V_COMP_ID);//V_COMP_ID
			...
			/*조회헤더*/
			if (U_TYPE.equals("H")) {
				cs = conn.prepareCall("call USP_003_M_CHARGE_RE_CALC(?,?,?,?,?,?,?,?)");
