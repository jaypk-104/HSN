...
<%
	...
	try {
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call USP_001_S_DN_REF_STEEL(?,?,?,?,?)");
			...
			while (rs.next()) {
				...
				subObject.put("DN_REAL_AMT", rs.getString("DN_REAL_AMT"));
				subObject.put("DN_ADD_PRC", rs.getString("DN_ADD_PRC"));
				subObject.put("DN_ADD_AMT", rs.getString("DN_ADD_AMT"));
			...
			cs = conn.prepareCall("call USP_001_S_BILL_H_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData);
			...
			cs = conn.prepareCall("call USP_001_A_TEMP_BN_FR_STEEL(?,?,?,?,?)");
