			...
			if (FileUpload.isMultipartContent(request)) {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField() && item.getSize() > 0) {
						...
						V_FILE_IN_SYSTEM_NM = V_ASN_NO + '_' + time + ext;
...
// 						File file = new File("C:\\OmegaPlus\\upload\\default\\AsnQcUploadFile\\" + V_FILE_IN_SYSTEM_NM);
// 						( 123.142.124.160 --> 43.254.152.57 ) 
		...
		/*파일삭제*/
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					V_ASN_NO = hashMap.get("ASN_NO") == null ? "" : hashMap.get("ASN_NO").toString();
				...
				V_ASN_NO = jsondata.get("ASN_NO") == null ? "" : jsondata.get("ASN_NO").toString();
