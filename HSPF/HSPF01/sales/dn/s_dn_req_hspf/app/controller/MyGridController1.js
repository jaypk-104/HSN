...
Ext.define('S_DN_REQ_HSPF.controller.MyGridController1', {
	...
	myGrid1CellDblClick: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		if(cellIndex == 14) { //입고일
			...
			var popup = Ext.create("S_DN_REQ_HSPF.view.WinGrPop");
			...
			if(Ext.getCmp('crudField').getValue() == 'C') {
				...
				store1.sync({
					params : {
						...
						V_VEH_INFO_REQUESTOR : Ext.getCmp('V_VEH_INFO_REQUESTOR').getValue(),
						V_VEH_INFO_REQUESTOR_TEL : Ext.getCmp('V_VEH_INFO_REQUESTOR_TEL').getValue(),
					},
					callback : function(records, operation, success) {
						...
						setTimeout(function() {
							...
							var myWin = new Ext.Window({
								title : '출고지시서',
								html : '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/on_server/DN_DIRECT_PRINT.jsp?reportParams=pdfEditable:true&V_TYPE=S&V_DD_NO='+V_DD_NO+'&V_REQ_TEXT='+V_DD_NO+'" width="100%" height="100%"></iframe>',
								width : 1000,
								height : 768,
								modal : true
							});
			...
			if(gridRecord[0].data['ITEM_CD'].substring(0,1) == 'B') {
				...
				if (gridRecord[0].get('DR_REQ_YN') == 'Y') {
					...
					Ext.MessageBox.confirm('확인', '총 [ '+ gridRecord.length + ' ] 건 출고요청하시겠습니까?', function(btn) {
						if(btn == 'yes') {
							var popup = Ext.create("S_DN_REQ_HSPF.view.MyWindow");
							popup.show();
			...
			if(gridRecord[0].data['DR_REQ_YN'] == 'Y') {
				Ext.Ajax.request({
					...
					success : function(response) {
						...
						Ext.getCmp('V_VEH_INFO_REQUESTOR').setValue(jsonResult.resultList["0"].VEH_INFO_REQUESTOR);
						Ext.getCmp('V_VEH_INFO_REQUESTOR_TEL').setValue(jsonResult.resultList["0"].VEH_INFO_REQUESTOR_TEL);
					},
					scope : this
				});
			}
		...
		comboStore.load({
			...
			callback : function(records, operations, success) {
				if (records.length > 0) {
					...
					Ext.getCmp('V_VEH_INFO_REQUESTOR').setValue(records[0].get('VEH_INFO_REQUESTOR'));
					Ext.getCmp('V_VEH_INFO_REQUESTOR_TEL').setValue(records[0].get('VEH_INFO_REQUESTOR_TEL'));
				}
			}
		});
	},
    ...
    poReqBtnClick : function(button, e, eOpts) {
		...
		if(gridRecord.length > 0) {
			...
			Ext.Msg.alert('확인', '발주요청 할 내역을 선택하세요.');
		}
