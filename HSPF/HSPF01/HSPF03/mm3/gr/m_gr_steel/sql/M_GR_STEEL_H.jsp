...
<%
	...
	try {
		...
		String V_ID_DT_FR = request.getParameter("V_ID_DT_FR") == null ? ""
				: request.getParameter("V_ID_DT_FR").toUpperCase().substring(0, 10);
		String V_ID_DT_TO = request.getParameter("V_ID_DT_TO") == null ? ""
				: request.getParameter("V_ID_DT_TO").toUpperCase().substring(0, 10);
		String V_BL_DOC_NO = request.getParameter("V_BL_DOC_NO") == null ? ""
				: request.getParameter("V_BL_DOC_NO").toUpperCase();
		String V_M_BP_NM = request.getParameter("V_M_BP_NM") == null ? ""
				: request.getParameter("V_M_BP_NM").toUpperCase();
		String V_SL_CD = request.getParameter("V_SL_CD") == null ? ""
				: request.getParameter("V_SL_CD").toUpperCase();
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call USP_001_M_GR_REF_STEEL(?,?,?,?,?,?,?)");
			...
			while (rs.next()) {
				...
				subObject.put("SPOT_XCH_AMT", rs.getString("SPOT_XCH_AMT"));
				
				System.out.println(rs.getString("SL_CD"));
...
// 			System.out.println(jsonObject);
			...
			String V_CC_NO = request.getParameter("V_CC_NO") == null ? ""
					: request.getParameter("V_CC_NO").toUpperCase();
			String V_CC_SEQ = request.getParameter("V_CC_SEQ") == null ? ""
					: request.getParameter("V_CC_SEQ").toUpperCase();
			String V_MVMT_DT_FR = request.getParameter("V_MVMT_DT_FR") == null ? ""
					: request.getParameter("V_MVMT_DT_FR").toUpperCase().substring(0, 10);
			String V_MVMT_DT_TO = request.getParameter("V_MVMT_DT_TO") == null ? ""
					: request.getParameter("V_MVMT_DT_TO").toUpperCase().substring(0, 10);

			String SQL = "SELECT   A.MVMT_NO, A.M_BP_CD, B.BP_NM M_BP_NM, A.IV_TYPE, D.DTL_NM IV_TYPE_NM, TO_CHAR(A.MVMT_DT, 'YYYY-MM-DD') MVMT_DT,                  ";
			SQL = SQL
					+ "                         I.LC_DOC_NO, G.BL_DOC_NO, '' PAY_TYPE_NM, A.ITEM_CD, C.ITEM_NM, ''SPEC,                                           ";
			SQL = SQL
					+ "                         A.CUR, C.UNIT, A.BON_QTY, A.BON_WGT_QTY BON_WGT_UNIT, A.BON_WGT_QTY * A.BON_QTY AS TOTAL_QTY, A.QTY,                           ";
			SQL = SQL
					+ "                         A.DOC_AMT, A.LOC_AMT, A.FORWARD_XCH_AMT, O.LO_OPEN_AMT LC_OPEN_AMT, L.DISTR_CC_AMT TONG_AMT, M.DISTR_TAX GWAN_AMT,";
			SQL = SQL
					+ "                         N.DISTR_REC INSU_AMT, Q.DISTR_ETC ETC_AMT, A.DISTR_AMT + NVL(ZZ.DISTR_AMT, 0) TOTAL_AMT, A.SL_CD, J.SL_NM, A.LOC_AMT AC_AMT,             ";
			SQL = SQL
					+ "                         A.FORWARD_XCH_AMT SL_AMT, A.CC_NO, A.CC_SEQ, ROUND(A.DOC_AMT/A.QTY,5) PRICE, A.PO_NO, A.PO_SEQ,                   ";
			SQL = SQL
					+ "                         A.XCHG_RT, K.USR_NM GR_USR_NM, A.FORWARD_XCH_RT, P.INSUR_AMT INSUR_AMT, R.LO_OPEN_AMT LC_AMEND_AMT,               ";
			SQL = SQL
					+ "                         CASE                                                                                                              ";
			SQL = SQL
					+ "                                  WHEN S.MVMT_NO IS NULL                                                                                   ";
			SQL = SQL
					+ "                                  THEN 'N'                                                                                                 ";
			SQL = SQL
					+ "                                  ELSE 'Y'                                                                                                 ";
			SQL = SQL
					+ "                         END DN_YN, A.SPOT_XCH_RT, A.SPOT_XCH_AMT, A.TEMP_GL_NO                                                                          ";
			SQL = SQL
					+ "                FROM     M_GR_MID_STEEL A                                                                                                  ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN B_BIZ_HSPF B                                                                                      ";
			SQL = SQL
					+ "                         ON       A.COMP_ID=B.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      A.M_BP_CD=B.BP_CD                                                                                        ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN B_ITEM_GROUP_STEEL C                                                                              ";
			SQL = SQL
					+ "                         ON       A.COMP_ID=C.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      A.ITEM_CD=C.ITEM_CD                                                                                      ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN B_DTL_INFO D                                                                                      ";
			SQL = SQL
					+ "                         ON       A.COMP_ID=D.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      A.IV_TYPE=D.DTL_CD                                                                                       ";
			SQL = SQL
					+ "                         AND      D.MAST_CD='MA02'                                                                                         ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CC_MID_STEEL E                                                                                  ";
			SQL = SQL
					+ "                         ON       A.COMP_ID=E.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      A.CC_NO  =E.CC_NO                                                                                        ";
			SQL = SQL
					+ "                         AND      A.CC_SEQ =E.CC_SEQ                                                                                       ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_BL_MID_STEEL F                                                                                  ";
			SQL = SQL
					+ "                         ON       E.COMP_ID=F.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      E.BL_NO  =F.BL_NO                                                                                        ";
			SQL = SQL
					+ "                         AND      E.BL_SEQ =F.BL_SEQ                                                                                       ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_BL_HDR_STEEL G                                                                                  ";
			SQL = SQL
					+ "                         ON       F.COMP_ID=G.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      F.BL_NO  =G.BL_NO                                                                                        ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_LC_DTL_STEEL H                                                                                  ";
			SQL = SQL
					+ "                         ON       F.COMP_ID=H.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      F.LC_NO  =H.LC_NO                                                                                        ";
			SQL = SQL
					+ "                         AND      F.LC_SEQ =H.LC_SEQ                                                                                       ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_LC_HDR_STEEL I                                                                                  ";
			SQL = SQL
					+ "                         ON       H.COMP_ID=I.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      H.LC_NO  =I.LC_NO                                                                                        ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN B_STORAGE_STEEL J                                                                                 ";
			SQL = SQL
					+ "                         ON       A.COMP_ID=J.COMP_ID                                                                                      ";
			SQL = SQL
					+ "                         AND      A.SL_CD  =J.SL_CD                                                                                        ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN Z_USR_INFO_HSPF K                                                                                 ";
			SQL = SQL
					+ "                         ON       A.INSRT_USR_ID = K.USR_ID                                                                                ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_CC L                                                                               ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = L.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_TAX M                                                                              ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = M.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_REC N                                                                              ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = N.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_LC_OPEN_ONLY O                                                                     ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = O.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_LC_OPEN_AMEND R                                                                    ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO=R.MVMT_NO                                                                                      ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_INS P                                                                              ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = P.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_CHARGE_STEEL_ETC Q                                                                              ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO = Q.MVMT_NO                                                                                    ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_GR_DN_STEEL_CHECK S                                                                             ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO=S.MVMT_NO                                                                                      ";
			SQL = SQL
					+ "                         LEFT OUTER JOIN M_GR_MID_STEEL_OMISSION ZZ                                                                             ";
			SQL = SQL
					+ "                         ON       A.MVMT_NO=ZZ.MVMT_NO AND A.GR_NO = ZZ.GR_NO and A.GR_SEQ = ZZ.GR_SEQ                                      ";
			...
			if (V_BL_DOC_NO.equals("")) {
				...
				SQL = SQL + " AND  G.BL_DOC_NO LIKE '%" + V_BL_DOC_NO + "%' " ;
			}
			SQL =  SQL  + " AND  A.MVMT_DT >= '"  + V_MVMT_DT_FR + "' ";
			SQL =  SQL  +  " AND  A.MVMT_DT <= '"  + V_MVMT_DT_TO + "' ";
			SQL =  SQL  +  " AND UPPER(B.BP_NM) LIKE UPPER('%" + V_M_BP_NM + "%') ";
			...
			sql += "         A.DOC_AMT, A.LOC_AMT, A.FORWARD_XCH_AMT, O.LO_OPEN_AMT LC_OPEN_AMT, L.STEEL_CC_AMT TONG_AMT, M.STEEL_TAX GWAN_AMT,";
			sql += "         N.STEEL_REC INSU_AMT, Q.STEEL_ETC ETC_AMT, A.STEEL_AMT TOTAL_AMT, A.SL_CD, J.SL_NM, A.LOC_AMT AC_AMT,             ";
			...
			/*
			cs = conn.prepareCall("call USP_001_M_GR_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
...
// 				subObject.put("GR_NO", rs.getString("GR_NO"));
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					String V_LOADING_DT = hashMap.get("LOADING_DT") == null ? ""
							: hashMap.get("LOADING_DT").toString().substring(0, 10);
					String V_INBOARD_DT = hashMap.get("INBOARD_DT") == null ? ""
							: hashMap.get("INBOARD_DT").toString().substring(0, 10);
					String V_ID_DT = hashMap.get("ID_DT") == null ? ""
							: hashMap.get("ID_DT").toString().substring(0, 10);
					...
					String V_XCHG_RT = hashMap.get("XCH_RATE") == null ? ""
							: hashMap.get("XCH_RATE").toString();
					...
					String V_BON_WGT_UNIT = hashMap.get("BON_WGT_UNIT") == null ? ""
							: hashMap.get("BON_WGT_UNIT").toString();
					String V_FORWARD_XCH_RT = hashMap.get("FORWARD_XCH_RT") == null ? ""
							: hashMap.get("FORWARD_XCH_RT").toString();
					String V_FORWARD_XCH_AMT = hashMap.get("FORWARD_XCH_AMT") == null ? ""
							: hashMap.get("FORWARD_XCH_AMT").toString();
					...
					if (V_TYPE.equals("IH") || V_TYPE.equals("ID")) {
						V_GR_NO = request.getParameter("V_GR_NO") == null ? ""
								: request.getParameter("V_GR_NO");
						V_MVMT_DT = request.getParameter("V_GR_DT") == null ? ""
								: request.getParameter("V_GR_DT").substring(0, 10);
					} else {
						V_GR_NO = hashMap.get("GR_NO") == null ? "" : hashMap.get("GR_NO").toString();
						V_GR_SEQ = hashMap.get("GR_SEQ") == null ? "" : hashMap.get("GR_SEQ").toString();
						V_MVMT_DT = hashMap.get("MVMT_DT") == null ? ""
								: hashMap.get("MVMT_DT").toString().substring(0, 10);
					}
					...
					String V_TEMP_PRC_YN = hashMap.get("TEMP_PRC_YN") == null ? ""
							: hashMap.get("TEMP_PRC_YN").toString();
					
					String V_SPOT_XCH_RT = hashMap.get("SPOT_XCH_RT") == null ? "" : hashMap.get("SPOT_XCH_RT").toString();
					String V_SPOT_XCH_AMT = hashMap.get("SPOT_XCH_AMT") == null ? "" : hashMap.get("SPOT_XCH_AMT").toString();

					cs = conn.prepareCall(
							"call USP_001_M_GR_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					cs.setString(1, V_COMP_ID);//V_COMP_ID
					cs.setString(2, V_TYPE);//V_TYPE
					...
					cs.setString(20, V_BON_QTY);//V_BOX_QTY
					cs.setString(21, V_BON_WGT_UNIT);//V_BOX_WGT_UNIT
					...
					cs.setString(29, V_SPOT_XCH_RT);//V_SPOT_XCH_RT
					cs.setString(30, V_SPOT_XCH_AMT);//V_SPOT_XCH_AMT
					cs.executeQuery();
					...
					if (V_TYPE.equals("ID")) {
						...
						cs2 = conn.prepareCall("call USP_001_M_CHARGE_CALC_2(?,?,?)");
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData);
				...
				String V_LOADING_DT = jsondata.get("LOADING_DT") == null ? ""
						: jsondata.get("LOADING_DT").toString().substring(0, 10);
				String V_INBOARD_DT = jsondata.get("INBOARD_DT") == null ? ""
						: jsondata.get("INBOARD_DT").toString().substring(0, 10);
				String V_ID_DT = jsondata.get("ID_DT") == null ? ""
						: jsondata.get("ID_DT").toString().substring(0, 10);
				...
				String V_BON_WGT_UNIT = jsondata.get("BON_WGT_UNIT") == null ? ""
						: jsondata.get("BON_WGT_UNIT").toString();
				String V_FORWARD_XCH_RT = jsondata.get("FORWARD_XCH_RT") == null ? ""
						: jsondata.get("FORWARD_XCH_RT").toString();
				String V_FORWARD_XCH_AMT = jsondata.get("FORWARD_XCH_AMT") == null ? ""
						: jsondata.get("FORWARD_XCH_AMT").toString();
				...
				if (V_TYPE.equals("IH") || V_TYPE.equals("ID")) {
					...
					V_MVMT_DT = jsondata.get("MVMT_DT") == null ? ""
							: jsondata.get("MVMT_DT").toString().substring(0, 10);
				}
				...
				String V_TEMP_PRC_YN = jsondata.get("TEMP_PRC_YN") == null ? ""
						: jsondata.get("TEMP_PRC_YN").toString();
				
				String V_SPOT_XCH_RT = jsondata.get("SPOT_XCH_RT") == null ? "" : jsondata.get("SPOT_XCH_RT").toString();
				String V_SPOT_XCH_AMT = jsondata.get("SPOT_XCH_AMT") == null ? "" : jsondata.get("SPOT_XCH_AMT").toString();
			...
			String V_MVMT_DT = request.getParameter("V_GR_DT") == null ? ""
					: request.getParameter("V_GR_DT").substring(0, 10);
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					cs = conn.prepareCall("call USP_001_M_CHARGE_CALC_2(?,?,?)");
					cs.setString(1, V_COMP_ID);//V_COMP_ID
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData);
			...
			String V_MVMT_NO = request.getParameter("V_MVMT_NO") == null ? ""
					: request.getParameter("V_MVMT_NO");

			// 			System.out.println("U_TYPE" + U_TYPE);
			// 			System.out.println("V_MVMT_NO" + V_MVMT_NO);

			/*-CHARGE_AMT :경비금액 , TOT_DISB_AMT :배분경비합계, 
			DISB_AMT 수정경비금액,ODD_DISB_AMT : 숨어있는 경비금액, STEELIBT_AMT :경비합계  */
			/*CHARGE_SEQ HIDDEN */

			/*조회헤더*/
			if (U_TYPE.equals("H")) {
				cs = conn.prepareCall("call USP_001_M_CHARGE_RE_CALC(?,?,?,?,?,?,?,?)");
				...
				while (rs.next()) {
					...
					subObject.put("OLD_DISB_AMT", rs.getString("OLD_DISB_AMT"));
					subObject.put("STEELIBT_AMT", rs.getString("STEELIBT_AMT"));
				...
				if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
					...
					for (int i = 0; i < jsonAr.size(); i++) {
						HashMap hashMap = (HashMap) jsonAr.get(i);
						V_TYPE = hashMap.get("V_TYPE") == null ? "" : hashMap.get("V_TYPE").toString();
						String V_CHARGE_NO = hashMap.get("CHARGE_NO") == null ? ""
								: hashMap.get("CHARGE_NO").toString();
						String V_CHARGE_SEQ = hashMap.get("CHARGE_SEQ") == null ? ""
								: hashMap.get("CHARGE_SEQ").toString();
						String V_DISTB_AMT = hashMap.get("DISTB_AMT") == null ? ""
								: hashMap.get("DISTB_AMT").toString();
...
// 					JSONObject jsondata = JSONObject.fromObject(jsonData);
					...
					V_TYPE = jsondata.get("V_TYPE") == null ? "" : jsondata.get("V_TYPE").toString();
					String V_CHARGE_NO = jsondata.get("CHARGE_NO") == null ? ""
							: jsondata.get("CHARGE_NO").toString();
					String V_CHARGE_SEQ = jsondata.get("CHARGE_SEQ") == null ? ""
							: jsondata.get("CHARGE_SEQ").toString();
					String V_DISTB_AMT = jsondata.get("DISTB_AMT") == null ? ""
							: jsondata.get("DISTB_AMT").toString();
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					HashMap hashMap = (HashMap) jsonAr.get(i);
					String V_TEMP_GL_NO = hashMap.get("TEMP_GL_NO") == null ? ""
							: hashMap.get("TEMP_GL_NO").toString();
					String V_GR_NO = hashMap.get("MVMT_NO") == null ? "" : hashMap.get("MVMT_NO").toString();
				...
				for (int j = 0; j < grArr.size(); j++) {
					if (!grArr_FIN.contains(grArr.get(j)))
						grArr_FIN.add(grArr.get(j));
				}

				for (int k = 0; k < grArr_FIN.size(); k++) {
					cs = conn.prepareCall("call USP_001_A_TEMP_GR_FR_STEEL(?,?,?,?,?)");
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData);
				...
				String V_GR_NO = jsondata.get("MVMT_NO") == null ? "" : jsondata.get("MVMT_NO").toString();
