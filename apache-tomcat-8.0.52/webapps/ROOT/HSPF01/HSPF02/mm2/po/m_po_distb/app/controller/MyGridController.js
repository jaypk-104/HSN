...
Ext.define('M_PO_DISTB.controller.MyGridController', {
	...
	gridAddBtnClick: function() {
		...
		if (flag == 'T') {
			var popup = Ext.create("M_PO_DISTB.view.WinAddRow");
			popup.show();
					    		...
					    		store.sync({
									...
									callback : function(records, operation, success) {
								    	...
								    	if(res2.match('Exception')) {
								    		...
								    		var tbController = M_PO_DISTB.app.getController("TbButtonController");
							...
							if(res.match('Exception')) {
								...
								if(res == 'Y') {	
									...
									var dynamicTab={
											...
											title : '담보등록',
											autoScroll: true,
											html : '<iframe name="xxx" border =0 src="/HSPF01/HSPF02/mm2/po/m_col_distb/M_COL_DISTB.jsp?PO_NO=12345" width="100%" height="100%"></iframe>',
											closable : true,
											id: Ext.getCmp('V_PO_NO').getValue()
									};
									parent.Ext.getCmp('myTab').add(dynamicTab).show();
									
//								Ext.getCmp('V_PO_NO').setValue(Ext.getCmp('V_PO_NO').getValue());
...
//				} else 
				...
				if (gridRecord[i].get('END_APPROV_NO') == undefined || gridRecord[i].get('END_APPROV_NO').indexOf('일반무역') == -1) {
					flag = 'F';
					msg = '취소품의번호를 입력하세요.'
					break;
				} else {
