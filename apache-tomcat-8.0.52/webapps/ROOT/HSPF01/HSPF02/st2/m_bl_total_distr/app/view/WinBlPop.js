/*
 * File: app/view/WinBlPop.js
 *
 * This file was generated by Sencha Architect version 4.2.2.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 6.5.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 6.5.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('M_BL_TOTAL_DISTR.view.WinBlPop', {
    extend: 'Ext.window.Window',
    alias: 'widget.winblpop',

    requires: [
    	'M_BL_TOTAL_DISTR.view.WinBlPopViewModel',
        'M_BL_TOTAL_DISTR.view.WinBlPopViewController',
        'Ext.form.FieldSet',
        'Ext.form.field.Date',
        'Ext.grid.Panel',
        'Ext.view.Table',
        'Ext.grid.column.RowNumberer',
        'Ext.grid.column.Date',
        'Ext.grid.column.Number',
        'Ext.form.field.Number',
        'Ext.selection.CheckboxModel',
        'Ext.grid.plugin.Exporter',
        'Ext.grid.plugin.RowEditing',
        'Ext.button.Button'
    ],

    viewModel: {
        type: 'winblpop'
    },
    id: 'WinBlPop',
    modal: true,
    height: 567,
    width: 1194,
    title: 'B/L조회',

    layout: {
        type: 'vbox',
        align: 'stretch'
    },
    items: [
        {
            xtype: 'container',
            modal: true,
            shim: false,
            padding: 10,
            layout: 'center',
            items: [
                {
                    xtype: 'fieldset',
                    height: 80,
                    maxHeight: 80,
                    minHeight: 80,
                    layout: {
                        type: 'hbox',
                        align: 'middle'
                    },
                    items: [
                        {
                            xtype: 'datefield',
                            flex: 1,
                            maxWidth: 220,
                            minWidth: 220,
                            width: 220,
                            fieldLabel: '선적일',
                            labelWidth: 80,
                            listeners : {
                                render : function(datefield) {
                                	var nows = new Date();
                                	nows.setMonth(nows.getMonth()-6);
//                                	nows.setDate(1);
                                    datefield.setValue(nows);
                                }	
                            },
                            format: 'Y-m-d',
                            id: 'W_LOADING_DT_FR'
                        },
                        {
                            xtype: 'datefield',
                            maxWidth: 150,
                            minWidth: 150,
                            width: 150,
                            fieldLabel: '~',
                            labelWidth: 10,
                            listeners : {
                                render : function(datefield) {
                                	var nows = new Date();
                                    datefield.setValue(nows);
                                }	
                            },
                            format: 'Y-m-d',
                            id: 'W_LOADING_DT_TO'
                        },
                        {
                            xtype: 'textfield',
                            flex: 1,
                            margin: '0 0 0 30',
                            maxWidth: 200,
                            minWidth: 200,
                            width: 200,
                            fieldLabel: '공급사명',
                            labelWidth: 80,
                            emptyText: '(Double Click)',
                            id: 'W_M_BP_NM_1',
                            emptyText: '(Double Click)',
                            listeners: {
                                afterrender: function(c) {
                                	c.getEl().on('dblclick', function() {
                            			var popup = Ext.create("Popup.view.WinMBpPop");
                            			popup.show();
                            			
                            			Ext.getCmp('fieldType').setValue('M_BL_DISTB_W');
                            			var store = Ext.getStore('WinMBpPopStore');
                            			store.removeAll();
                                    });
                                }
                            },
                            name: 'bl_search'
                        },
                        {
                            xtype: 'textfield',
                            flex: 1,
                            margin: '0 0 0 30',
                            maxWidth: 200,
                            minWidth: 200,
                            width: 200,
                            fieldLabel: '수주처명',
                            labelWidth: 80,
                            id: 'W_M_BP_NM2',
//                            emptyText: '(Double Click)',
//                            listeners: {
//                                afterrender: function(c) {
//                                	c.getEl().on('dblclick', function() {
//                            			var popup = Ext.create("Popup.view.WinMBpPop");
//                            			popup.show();
//                            			
//                            			Ext.getCmp('fieldType').setValue('M_IV_DISTR_W');
//                            			var store = Ext.getStore('WinMBpPopStore');
//                            			store.removeAll();
//                                    });
//                                }
//                            },
                            name: 'bl_search'
                        },
                        {
                        	xtype: 'textfield',
                        	flex: 1,
                        	margin: '0 0 0 30',
                        	maxWidth: 230,
                        	minWidth: 230,
                        	width: 230,
                        	fieldLabel: 'L/C번호',
                        	labelWidth: 80,
                        	name: 'bl_search',
                        	id: 'W_LC_DOC_NO'
                        },
                        {
                            xtype: 'textfield',
                            flex: 1,
                            margin: '0 0 0 30',
                            maxWidth: 220,
                            minWidth: 220,
                            width: 220,
                            fieldLabel: 'B/L번호',
                            labelWidth: 80,
                            name: 'bl_search',
                            id: 'W_BL_DOC_NO'
                        },
                        {
                            xtype: 'textfield',
                            flex: 1,
                            margin: '0 0 0 30',
                            maxWidth: 220,
                            minWidth: 220,
                            width: 220,
                            fieldLabel: '발주번호',
                            labelWidth: 80,
                            name: 'bl_search',
                            id: 'W_PO_NO'
                        }
                    ]
                }
            ]
        },
        {
            xtype: 'container',
    		flex : 1,
    		layout : 'fit',
            items: [
                {
                    xtype: 'gridpanel',
                    style: 'padding: 10px;',
                    bodyBorder: false,
                    header: false,
                    store: 'PopStore',
                    id: 'popGrid',
                    viewConfig: {
                    	enableTextSelection: true,
                    },
                    columns: [
                        {
                            xtype: 'rownumberer',
                            width: 40
                        },
                        {
                            xtype: 'gridcolumn',
                            hidden: true,
                            text: 'V_TYPE'
                        },
                        {
                            xtype: 'gridcolumn',
                            style: 'text-align: center; font-weight: bold;',
                            sortable: true,
                            dataIndex: 'BL_NO',
                            enableTextSelection: true,
                            text: 'B/L관리번호',
                            width : 150
                        },
                        {
                            xtype: 'gridcolumn',
                            style: 'text-align: center; font-weight: bold;',
                            sortable: true,
                            dataIndex: 'BL_DOC_NO',
                            enableTextSelection: true,
                            text: 'B/L번호',
                            width : 150
                        },
                        {
                        	xtype: 'gridcolumn',
                        	style: 'text-align: center; font-weight: bold;',
                        	sortable: true,
                        	dataIndex: 'LC_DOC_NO',
                        	enableTextSelection: true,
                        	text: 'L/C번호',
                        	width : 160
                        },
                        {
                        	xtype: 'gridcolumn',
                        	style: 'text-align: center; font-weight: bold;',
                        	sortable: true,
                        	dataIndex: 'SO_BP_NM',
                        	enableTextSelection: true,
                        	text: '수주처',
                        	width : 180
                        },
                        {
                            xtype: 'datecolumn',
                            style: 'text-align: center; font-weight: bold;',
                            align: 'center',
                            dataIndex: 'LOADING_DT',
                            enableTextSelection: true,
                            text: '선적일',
                            format: 'Y-m-d',
                        },
                        {
                            xtype: 'gridcolumn',
                            style: 'text-align: center; font-weight: bold;',
                            sortable: true,
                            dataIndex: 'IN_TERMS_NM',
                            enableTextSelection: true,
                            text: '가격조건',
                            width : 200
                        },
                        {
                            xtype: 'gridcolumn',
                            style: 'text-align: center; font-weight: bold;',
                            sortable: true,
                            dataIndex: 'PAY_METH_NM',
                            enableTextSelection: true,
                            text: '결제방법',
                            width : 200
                        },
                        {
                            xtype: 'numbercolumn',
                            style: 'text-align: center; font-weight: bold;',
                            align: 'end',
                            dataIndex: 'XCH_RATE',
                            enableTextSelection: true,
                            text: '환율',
                            format: '0,000.00',
                        },
                        {
                            xtype: 'gridcolumn',
                            style: 'text-align: center; font-weight: bold;',
                            sortable: true,
                            dataIndex: 'CUR',
                            enableTextSelection: true,
                            text: '통화',
                        },
                        {
                            xtype: 'numbercolumn',
                            style: 'text-align: center; font-weight: bold;',
                            align: 'end',
                            dataIndex: 'DOC_AMT',
                            enableTextSelection: true,
                            text: '총 금액',
                            format: '0,000.00',
                            width: 120
                        },
                        {
                            xtype: 'numbercolumn',
                            style: 'text-align: center; font-weight: bold;',
                            align: 'end',
                            dataIndex: 'LOC_AMT',
                            enableTextSelection: true,
                            text: '총 자국금액',
                            format: '0,000',
                            width: 120
                        }
                    ],
                    selModel: {
                        selType: 'rowmodel',
                        selMode: 'SINGLE'
//                        checkOnly: true
                    },
                    plugins: [
                        {
                            ptype: 'gridexporter'
                        },
                    ]
                }
            ]
        },
        {
            xtype: 'container',
            margin: '0 0 3 0',
            layout: {
                type: 'hbox',
                align: 'stretch',
                pack: 'center'
            },
            items: [
                {
                    xtype: 'button',
                    height: 30,
                    id: 'w_blSelBtn',
                    maxHeight: 30,
                    minHeight: 30,
                    width: 100,
                    text: '조회'
                },
                {
                    xtype: 'button',
                    margins: '',
                    height: 30,
                    id: 'w_blRegBtn',
                    margin: '0 0 0 3',
                    maxHeight: 30,
                    minHeight: 30,
                    width: 100,
                    text: '선택'
                }
            ]
        }
    ]

});