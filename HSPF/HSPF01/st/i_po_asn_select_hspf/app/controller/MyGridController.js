...
Ext.define('I_PO_ASN_SELECT.controller.MyGridController', {
    ...
    barcdPrintBtnClick : function(button, e, eOpts) {
    	var grid = Ext.getCmp('myGrid');
    	var store = grid.getStore();
    	var record = grid.getSelectionModel().getSelection();
    	
    	if(record.length >= 1){
    		if(record[0].data['V_TYPE'] == 'V'){
    			var lot_no = record[0].data['LOT_BC_NO'];
        		record[0].set('V_TYPE', 'P');
    		}
    		
    		for( var i = 1 ; i < record.length ; i ++){
    			if(record[i].data['V_TYPE'] == 'V'){
    				record[i].set('V_TYPE', 'P');
    				lot_no += '<>';
