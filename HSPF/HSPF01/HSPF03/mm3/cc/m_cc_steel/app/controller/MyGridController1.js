...
Ext.define('M_CC_STEEL.controller.MyGridController1', {
...
//			msg = '선물환율을 입력하세요.';
		...
		else if (Ext.getCmp('V_CY_DT').getValue() == null || Ext.getCmp('V_CY_DT').getValue() == undefined || Ext.getCmp('V_CY_DT').getValue() == '') {
			flag = 'F';
			msg = '입항일을 입력하세요.';
		}
		else if (Ext.getCmp('V_FORWARD_XCH').getValue() > Ext.getCmp('V_SPOT_XCH').getValue()){
			flag = 'F';
			msg = '현물환율 금액은 선물환율 금액보다 커야합니다.';
		}
		else {
			flag = 'T';
		}
		...
		store1.each(function(record, idx) {
			if(record.get('CHARGE_YN') == 'Y') {
				flag = 'F';
				msg = '경비가 존재합니다. 변경할 수 없습니다.';
				return false;
			} 
			else if(record.get('GR_YN') == 'Y') {
				flag = 'F';
				msg = '입고가 존재합니다. 변경할 수 없습니다.';
				return false;
			} 
		});
					...
					if (button == 'yes') {
			    		Ext.Ajax.request({
			    			...
			    			params: {
			    				...
			    				V_SPOT_XCH : Ext.getCmp('V_SPOT_XCH').getValue(), //현물환율
			    				...
			    				V_CY_DT : Ext.getCmp('V_CY_DT').getValue(), //입항일
			    			},
...
//			    				if(res.match('V') || res.match('CU')) {
			    					...
			    					store1.sync({ 
			    						...
			    						callback : function(records, operation, success) {
								    		var tbController = M_CC_STEEL.app.getController("TbButtonController");
