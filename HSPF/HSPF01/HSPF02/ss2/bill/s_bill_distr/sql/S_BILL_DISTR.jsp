...
<%
	...
	try {
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call DISTR_BILL.USP_S_DN_REF_DISTR(?,?,?,?,?)");
			...
			cs = conn.prepareCall("call DISTR_BILL.USP_S_BILL_H_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
...
// 						System.out.println(jsonObject);
			...
			cs = conn.prepareCall("call DISTR_BILL.USP_S_BILL_D_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call DISTR_TEMP_GL.USP_A_TEMP_BN_FR(?,?,?,?,?)");
