...
<%
	...
	try {
		...
		if (V_TYPE.equals("S")) {
			...
			cs = conn.prepareCall("call USP_001_M_CC_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			while (rs.next()) {
				...
				subObject.put("BON_WGT_UNIT", rs.getString("BON_WGT_UNIT"));//.BOX_WGT_UNIT
			...
			cs = conn.prepareCall("call USP_001_M_CC_POPUP_STEEL(?,?,?,?,?,?)");
			...
			cs.setString(6, V_SO_BP_NM);//V_SO_BP_NM
			cs.executeQuery();
...
// 			System.out.println(jsonObject);
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					String V_BON_QTY = hashMap.get("BON_QTY") == null ? "" : hashMap.get("BON_QTY").toString();//BOX_QTY
					String V_BON_WGT_QTY = hashMap.get("BON_WGT_UNIT") == null ? "" : hashMap.get("BON_WGT_UNIT").toString();//BOX_WGT_UNIT
					...
					String V_TAX_RATE = hashMap.get("TAX_RT") == null ? "0" : hashMap.get("TAX_RT").toString();
					String V_SPOT_XCH_AMT = hashMap.get("SPOT_XCH_AMT") == null ? "0" : hashMap.get("SPOT_XCH_AMT").toString();
					...
					//cs = conn.prepareCall("call USP_001_M_CC_DTL_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					cs = conn.prepareCall("call USP_001_M_CC_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					cs.setString(1, V_TYPE);//V_TYPE
					cs.setString(2, V_COMP_ID);//V_COMP_ID
					cs.setString(3, V_CC_NO);//V_CC_NO
					...
					cs.setString(12, V_BON_QTY);//V_BOX_QTY
					cs.setString(13, V_BON_WGT_QTY);//V_BOX_WGT_UNIT
					...
					cs.setString(22, V_SPOT_XCH_AMT);//V_SPOT_XCH_AMT
					cs.executeQuery();
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData); //큰수에 소수점이 있는경우 숫자계산이 이상해서 수정함. 20200108 김장운
				...
				V_CC_NO = jsondata.get("CC_NO") == null ? "" : jsondata.get("CC_NO").toString(); //GR
				//V_CC_NO = request.getParameter("V_CC_NO") == null ? "" : request.getParameter("V_CC_NO").toUpperCase();
				...
				String V_BON_QTY = jsondata.get("BON_QTY") == null ? "" : jsondata.get("BON_QTY").toString();//BOX_QTY
				String V_BON_WGT_QTY = jsondata.get("BON_WGT_UNIT") == null ? "" : jsondata.get("BON_WGT_UNIT").toString();//BOX_WGT_UNIT
				String V_GR_QTY = jsondata.get("GR_QTY") == null ? "0" : jsondata.get("GR_QTY").toString(); //0
				...
				String V_TAX_RATE = jsondata.get("TAX_RT") == null ? "0" : jsondata.get("TAX_RT").toString();
				String V_SPOT_XCH_AMT = jsondata.get("SPOT_XCH_AMT") == null ? "0" : jsondata.get("SPOT_XCH_AMT").toString();
...
// 				System.out.println("V_TYPE : " + V_TYPE);
// 				System.out.println("V_BON_QTY : " + V_BON_QTY);
// 				System.out.println("V_BON_WGT_QTY : " + V_BON_WGT_QTY);

				//cs = conn.prepareCall("call USP_001_M_CC_DTL_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");GR
				cs = conn.prepareCall("call USP_001_M_CC_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				cs.setString(1, V_TYPE);//V_TYPE
				cs.setString(2, V_COMP_ID);//V_COMP_ID
				cs.setString(3, V_CC_NO);//V_CC_NO
