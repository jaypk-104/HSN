...
<%
	...
	try {
		...
		//관리구분팝업조회
		if (V_TYPE.equals("SP")) {
			cs = conn.prepareCall("call USP_001_M_CHARGE_POPUP_STEEL(?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call USP_001_M_CHARGE_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call USP_001_M_CHARGE_CALC_1(?,?,?,?,?,?,?)");
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (V_EXP_ITEM_FLG.equals("true") || V_EXP_ITEM_FLG.equals("Y")) {
						...
						cs2 = conn.prepareCall("call USP_001_M_CHARGE_CALC_1(?,?,?,?,?,?,?)");
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (V_TYPE.equals("I")) {
						...
						cs = conn.prepareCall("call USP_001_TEMP_M_CHARGE_NO_STEEL(?,?)");
						...
						/*제일 마지막 한번*/
						if (i == jsonAr.size() - 1) {
							cs = conn.prepareCall("call USP_001_A_TEMP_CH_STEEL_FR(?,?,?,?,?,?,?,?)");
							cs.setString(1, V_COMP_ID);//V_COMP_ID
