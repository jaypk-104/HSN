...
Ext.define('SUPP_DLV_TOT_HSPF.controller.AsnPopController', {
    ...
    control: {
        ...
        "#delAsnItemBtn": { /*ASN품목하나삭제*/
        	click: 'delAsnItemBtnClick'
        },
        "#stsPrtBtn": { /*거래명세서*/
    ...
    /* 납기현황등록 팝업 */
	/* MyStore : [ SUPP_DLV_TOT_HSPF_H ] */
	/* MyStore1 : [ SUPP_DLV_TOT_HSPF_H ] */
    /* MyStore2 : [ SUPP_DLV_TOT_HSPF_D ] */
    /* MyStore3 : [ SUPP_DLV_TOT_HSPF_D ] */
    ...
    creBcdBtnClick: function() {
    	...
    	if(flag == 'T') {
	    	Ext.Ajax.request({
	    		url: 'sql/SUPP_DLV_TOT_HSPF_D.jsp',
	    		method: 'post',
	    		params: {
	    			V_TYPE: 'CB',
    ...
    divBacdBtn2Click: function(button, e, eOpts) {
        ...
        if(flag == 'T') {
        	var popup = Ext.create("SUPP_DLV_TOT_HSPF.view.WinAddRow");
            popup.show();
        } else {
    	...
    	if(flag == 'T') {
    		Ext.Ajax.request({
    			...
    			callback : function(records,operations,success){
    				...
    				if(Ext.getCmp('W_SYS_TYPE').getValue() == 'C'){
    					//자동차 부품
    					if(MAIN_SERVER_YN == 'Y'){
    						html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/SWM_BAR_CODE.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
    					}
						else{
							html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/SWM_BAR_CODE_TEST.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
						}
    				}
    				else if (Ext.getCmp('W_SYS_TYPE').getValue() == 'E'){
    					//원자재
    					if(MAIN_SERVER_YN == 'Y'){
    						...
    						html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/HSPF_BAR_CODE_TEST.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
    					}
    				}
    	...
    	if(flag == 'T') {
    		Ext.Ajax.request({
    			...
    			callback : function(records,operations,success){
    				...
    				console.log(Ext.getCmp('W_SYS_TYPE').getValue());
    				console.log(MAIN_SERVER_YN);
    				
    				if(Ext.getCmp('W_SYS_TYPE').getValue() == 'C'){
    					//자동차 부품
    					if(MAIN_SERVER_YN == 'Y'){
    						html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/SWM_BAR_CODEX4.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
    					}
						else{
							html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/SWM_BAR_CODEX4_TEST.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
						}
    				}
    				else if (Ext.getCmp('W_SYS_TYPE').getValue() == 'E'){
    					//원자재
    					if(MAIN_SERVER_YN == 'Y'){
    						...
    						html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/HSPF_BAR_CODEX4_TEST.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>';
    					}
    				}
...
//    	
    	if(flag1 == 'T' && flag2 =='T') { //바코드미확정, 로트정보입력완료
    		var myMask = new Ext.LoadMask(Ext.getCmp('myGrid3') , {msg:"Please wait..."});
    		myMask.show();
    		store3.sync({
    			params: {
    				V_TYPE: 'LOT',
...
//    		}
    ...
    delAsnItemBtnClick: function() {
    	...
    	var record = Ext.getCmp('myGrid2').getSelectionModel().getSelection();
    	
    	console.log(record);
    	console.log(record[0]);
    	
    	if(record.length == 0){
    		Ext.getCmp('확인', '삭제할 데이터를 선택하세요.');
    	}
    	else{
    		Ext.Ajax.request({
        		url : 'sql/SUPP_DLV_TOT_HSPF_H.jsp',
        		method : 'post',
        		params : {
        			V_TYPE : 'CHECK',
        			V_ASN_NO : record[0].data['ASN_NO'],
        			V_PO_NO : record[0].data['PO_NO'],
        			V_PO_SEQ : record[0].data['PO_SEQ'],
        		},
        		success : function(response) {
//        			var V_BILL_NO = Ext.JSON.decode(response.responseText);
        			console.log(response);
        			
        			var resultStatus = response.responseText;
        			
        			if(resultStatus == 'OK'){
        				Ext.MessageBox.confirm('확인', '품목(' + record[0].data['ITEM_NM'] + ') 삭제 하시겠습니까?', function(btn) {
        				...
        				Ext.Msg.alert('확인', '삭제할 수 없습니다. 담당자에게 문의하세요.');
        			}
        			
        			/*
        			Ext.getCmp('u_bill_no').setValue(V_BILL_NO.BILL_NO);
        			store.sync({
        				params : {
        					V_BILL_NO : V_BILL_NO,
        					V_BILL_DT : Ext.getCmp('bill_dt').getValue(),
        					V_USR_ID : parent.Ext.getCmp('GUSER_ID').getValue()
        				},
        				callback : function(batch) {
        	    			...
        	    			if( response.indexOf('{') != -1){
        	    				...
        	    				var success = jResult.success;
        	    				var errMsg = jResult.resultList;
        	    				if( success == false){
        	    					Ext.Msg.alert('확인', '관리자에게 문의 해주십시오.<br>오류메시지 :<br>' + errMsg);
        	    				}
        	    				myMask.hide();
        	    			}
        				}
    ...
    stsPrtBtnClick: function() {
    	...
    	var html = '';
    	if(MAIN_SERVER_YN == 'Y'){
    		html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/HSPF_RECEIPT.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>'
    	}
    	else{
    		html = '<iframe name="xxx" border =0 src="http://123.142.124.170:8080/aireport/TOT_HSPF/HSPF_RECEIPT_TEST.jsp?V_ASN_NO='+ASN_NO+'" width="100%" height="100%"></iframe>'
    	}
    	var myWin=new Ext.Window(
			{
				title : '거래명세표',
    ...
    fileAddBtnClick: function() {
    	var popup = Ext.create("SUPP_DLV_TOT_HSPF.view.MyWindow");
		popup.show();
    ...
    fileGridDblClick:  function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		var myWin=new Ext.Window(
				{
					title : '파일다운로드',
					html : '<iframe name="xxx" border =0 src="sql/SUPP_DLV_TOT_HSPF_FILE.jsp?V_TYPE=D&V_FILE_NM='+V_FILE_NM+'&V_FILE_IN_SYSTEM_NM='+V_FILE_IN_SYSTEM_NM+'" width="1%" height="1%"></iframe>',
					width :500,
					height:500
				});
		myWin.show();
	    	...
	    	if(gridRecord.length == 1) {
	    		Ext.Ajax.request({
	    		    ...
	    		    success : function(response) {
	    		    	...
	    		    	var rec = Ext.create('SUPP_DLV_TOT_HSPF.model.MyModel', {
	...
	grdnBtnClick: function(button, e, eOpts) {
		var popup = Ext.create("SUPP_DLV_TOT_HSPF.view.WinGrDn");
		popup.show();
