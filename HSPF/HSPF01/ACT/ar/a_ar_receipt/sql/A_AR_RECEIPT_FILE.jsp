...
<%
	...
	try {
		...
		String V_CLS_AR_NO = request.getParameter("V_CLS_AR_NO") == null ? "" : request.getParameter("V_CLS_AR_NO");
			...
			if (FileUpload.isMultipartContent(request)) {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField() && item.getSize() > 0) {
						...
						V_FILE_IN_SYSTEM_NM = V_CLS_AR_NO + '_' + time + ext;

						//로컬용
						//File file = new File("\\\\123.142.124.137\\aiusr\\webapps\\hsnelec\\ROOT\\aireport\\AIViewer55\\HSPF01\\FILEUPLOAD\\" + V_FILE_IN_SYSTEM_NM);

						//서버용
						File file = new File("/data/a_ar_receipt/" + V_FILE_IN_SYSTEM_NM);
						
						request.setAttribute("file", file);
						item.write(file);
						item.delete();
 									
						cs = conn.prepareCall("call USP_A_AR_RECEIPT_FILE_HSPF(?,?,?,?,?,?,?,?,?)");
						cs.setString(1, V_COMP_ID);
						cs.setString(2, V_TYPE);
						cs.setString(3, V_CLS_AR_NO);
						cs.setString(4, V_TEMP_GL_NO);
						cs.setString(5, V_SEQ);
						cs.setString(6, V_FILE_NM);
						cs.setString(7, V_FILE_IN_SYSTEM_NM);
						cs.setString(8, V_USR_ID);
						cs.registerOutParameter(9, com.tmax.tibero.TbTypes.CURSOR);
						cs.executeQuery();
						
 						e_cs = e_conn.prepareCall("{call USP_A_AR_RECEIPT_FILE_UPLOAD_ERP(?,?,?,?,?)}");
						e_cs.setString(1, "I");
						e_cs.setString(2, V_CLS_AR_NO);
						e_cs.setString(3, V_TEMP_GL_NO);
						e_cs.setString(4, V_SEQ);
						e_cs.setString(5, V_USR_ID);
						e_cs.execute();
...
// 			String savePath = "/data/apache/webapps/ROOT/HSPF01_FILE/";
			...
			String savePath = "/data/a_ar_receipt/";
		...
		/*파일삭제*/
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					V_CLS_AR_NO = hashMap.get("CLS_AR_NO") == null ? "" : hashMap.get("CLS_AR_NO").toString();
				...
				V_CLS_AR_NO = jsondata.get("CLS_AR_NO") == null ? "" : jsondata.get("CLS_AR_NO").toString();
