	...
	*/
	try {
		...
		B_TYPE = request.getParameter("B_TYPE");
 		System.out.println("B_TYPE2 : " + B_TYPE);
...
// 			V_PASSWORD = j.get("PWD").toString();
			...
			cs = conn137.prepareCall("call USP_BARCODE(?,?,?,?,?,?,?,?,?,?)");
			...
			cs = conn137.prepareCall("call USP_M_GR_BARCODE(?,?,?,?,?,?,?,?,?,?)");
			...
			for (int i = 0; i < barArr.length; i++) {
				if (i == 0) {
					cs = conn137.prepareCall("call USP_M_GR_BARCODE_AUTONUM(?,?,?,?)");
				...
				System.out.println("GR_NO =  " + V_GR_NO);
			...
			cs = conn137.prepareCall("call USP_DIREC_GR_DN_BARCODE(?,?,?,?,?,?)");
			cs.setString(1, V_TYPE);//V_TYPE
			cs.setString(2, V_COMP_ID);//V_COMP_ID
			...
			cs = conn137.prepareCall("call USP_M_RETURN_BCD(?,?,?,?,?,?)");
			...
			cs = conn137.prepareCall("call USP_S_DN_BARCODE(?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, "DD");
			cs.setString(2, "");
...
// 			System.out.println(V_ITEM_CD);
		...
		//바코드 분할 바코드 조회
			...
			cs = conn137.prepareCall("call USP_I_BARCODE_DIVISION(?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, "");
		...
		//재고이동 바코드 조회
			...
			cs = conn137.prepareCall("call USP_I_STOCK_MOVE_STORAGE(?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_TYPE);
			cs.setString(2, "");
			cs.setString(3, "");
		...
		//출고반품바코드조회
			...
			cs = conn137.prepareCall("call USP_S_RETURN_BCD(?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_TYPE);
			cs.setString(2, "");
			cs.setString(3, "");
			cs.setString(4, V_BAR_NO);
		...
		//출고지시서 확인
			...
			jsonObject = new JSONObject();
			cs = conn137.prepareCall("call USP_GET_BARCODE_INFO(?,?,?)");
		...
		//재고 조사
			...
			String V_USR_ID = j.get("USR_ID").toString().toUpperCase();
			jsonObject = new JSONObject();
			cs = conn137.prepareCall("call USP_GET_MVMT_BARCODE_INFO(?,?,?,?,?,?)");
			cs.setString(1, "S");
			cs.setString(2, V_COMP_ID);
		...
		//입고예정정보
			...
			cs = conn137.prepareCall("call USP_GET_GRSCHEDUL_LIST(?,?,?,?,?,?)");
			cs.setString(1, "S");
			cs.setString(2, V_COMP_ID);
