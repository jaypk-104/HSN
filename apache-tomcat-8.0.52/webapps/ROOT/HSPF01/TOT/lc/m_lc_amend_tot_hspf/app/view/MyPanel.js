...
Ext.define('M_LC_AMEND_TOT_HSPF.view.MyPanel', {
	extend : 'Ext.panel.Panel',
	alias : 'widget.mypanel',

	requires : [ 'M_LC_AMEND_TOT_HSPF.view.MyPanelViewModel', 
				'Ext.grid.Panel', 
...
//							Ext.getCmp('V_IN_TERMS').setValue(record.get('IN_TERMS'));
							...
							Ext.getCmp('V_OPEN_DT').setValue(record.get('OPEN_DT'));
...
//							flag = 'F';
//							msg = '결제방법이 일치하지 않습니다.';
						} else if (LC_DOC_NO != record.get('LC_DOC_NO')) {
							flag = 'F';
							msg = '동일한 L/C를 선택하세요.';
						} else {
							flag = 'T';
						...
						if (flag == 'T') {
							store1.insert(store1Cnt, [ ({
								...
								BE_QTY : record.get('QTY'),
								AT_QTY : record.get('QTY'),
								BE_PRICE : record.get('PRICE'),
								AT_PRICE : record.get('PRICE'),
								BE_DOC_AMT : record.get('DOC_AMT'),
								AT_DOC_AMT : record.get('DOC_AMT'),
								BE_LOC_AMT : record.get('LOC_AMT'),
								AT_LOC_AMT : record.get('LOC_AMT'),
					...
					deselect : function(rowmodel, record, index, eOpts) {
						...
						store1.each(function(rec, idx) {
							if (rec.get('LC_NO') == record.get('LC_NO') && rec.get('LC_SEQ') == record.get('LC_SEQ')) {
								store1.remove(rec);
							}
						});
