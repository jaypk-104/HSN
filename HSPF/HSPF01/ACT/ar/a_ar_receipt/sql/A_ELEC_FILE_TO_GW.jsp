			...
			if (FileUpload.isMultipartContent(request)) {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField() && item.getSize() > 0) {
						...
						V_FILE_IN_SYSTEM_NM = V_TEMP_GL_NO + '_' + time + ext;

						//로컬용
// 						File file = new File("D:\\TEST\\" + V_FILE_IN_SYSTEM_NM);

						//서버용
						File file = new File("/data/ELEC/" + V_FILE_IN_SYSTEM_NM);
						
						request.setAttribute("file", file);
						item.write(file);
						item.delete();
						
						cs = conn.prepareCall("call USP_A_ELEC_FILE_TO_GW(?,?,?,?,?,?,?,?,?,?)");
						cs.setString(1, V_COMP_ID);
						cs.setString(2, V_TYPE);
						cs.setString(3, V_TEMP_GL_NO);
						cs.setString(4, "");
						cs.setString(5, V_FILE_NM);
						cs.setString(6, V_FILE_IN_SYSTEM_NM);
						cs.setString(7, ext);
						cs.setString(8, String.valueOf(file.length()));
