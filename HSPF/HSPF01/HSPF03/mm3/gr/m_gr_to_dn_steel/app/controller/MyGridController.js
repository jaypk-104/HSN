...
Ext.define('M_GR_TO_DN_STEEL.controller.MyGridController', {
    ...
    gridDnBtnClick : function(button, e, eOpts) {
		...
		for(var i=0; i<gridRecord.length; i++) {
			...
			else if((gridRecord[i].data['FORWARD_XCH_RT'] == 1 || gridRecord[i].data['FORWARD_XCH_RT'] == '' || gridRecord[i].data['FORWARD_XCH_RT'] == null || gridRecord[i].data['FORWARD_XCH_RT'] == undefined) && gridRecord[i].data['IV_TYPE_NM'] == '수입' ) {
				flag = 'F';
				msg = '통관등록에서 선물환율을 입력해주세요.';
				break;
			}
			else {
				flag = 'T';
    ...
    myGridClick : function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		for(var i=0; i<gridRecord.length; i++) {
			if(!mvmtArr.includes(gridRecord[i].get('MVMT_NO') + gridRecord[i].get('MVMT_SEQ'))) {
				mvmtArr.push(gridRecord[i].get('MVMT_NO') + gridRecord[i].get('MVMT_SEQ'));
			}
		}
		...
		if (record.get('DN_BON_QTY') > 0 && (gridRecord.length > 0)) {
	...
	addColBtnClick: function(button, e, eOpts) {
		...
		if(gridRecord.length == 1) {
			var popup = Ext.create("M_GR_TO_DN_STEEL.view.MyWindow3");
			...
			Ext.Ajax.request({
				...
				success : function(response) {
					...
					if(colStore.getCount() == 0) {
				    	...
				    	var rec = Ext.create('M_GR_TO_DN_STEEL.model.MyModel', {
