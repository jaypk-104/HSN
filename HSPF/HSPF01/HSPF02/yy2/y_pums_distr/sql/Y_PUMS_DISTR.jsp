...
<%
	...
	try {
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call DISTR_E_RISK.USP_E_RISK_HD_DITR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			String W_BP_NM = request.getParameter("W_BP_NM") == null ? "" : request.getParameter("W_BP_NM").toUpperCase();

			// 			System.out.println(W_RISK_TYPE);
			...
			cs.setString(2, V_COMP_ID);//V_COMP_ID
			cs.setString(3, V_RISK_NO);//V_RISK_NO
			cs.setString(4, W_BP_NM);//V_RISK_DOC_NO //왜 여기에 판매처명을 넣는지는 모르겠음..
			cs.setString(5, "");//V_DEPT_CD
			...
			String V_MID_RATE = request.getParameter("V_MID_RATE") == null ? "" : request.getParameter("V_MID_RATE").toUpperCase();
			...
			String V_RISK_YN = request.getParameter("V_RISK_YN").toString().toLowerCase().equals("true") ? "Y" : "N";

			// 			System.out.println("V_RISK_YN" + request.getParameter("V_RISK_YN"));
			// 			System.out.println("V_RISK_YN" + V_RISK_YN);
			...
			if (V_TYPE.equals("S")) {
				...
				cs = conn.prepareCall("call DISTR_E_RISK.USP_E_RISK_CR_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?)");
				cs.setString(1, V_TYPE);//V_TYPE
				cs.setString(2, V_COMP_ID);//V_COMP_ID
				cs.setString(3, V_RISK_NO);//V_RISK_NO
				...
				while (rs.next()) {
					...
					subObject.put("TERMS", rs.getString("TERMS"));
					subObject.put("REMARK", rs.getString("REMARK"));
					jsonArray.add(subObject);
				}
			...
			//계약금 SYNC
				...
				if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
					...
					for (int i = 0; i < jsonAr.size(); i++) {
						...
						String V_TERMS = hashMap.get("TERMS") == null ? "" : hashMap.get("TERMS").toString();
						...
						cs.setString(13, V_TERMS);//V_TERMS
						cs.executeQuery();
					...
					String V_TERMS = jsondata.get("TERMS") == null ? "" : jsondata.get("TERMS").toString();
			...
			if (V_TYPE.equals("S")) {
				...
				cs = conn.prepareCall("call DISTR_E_RISK.USP_E_RISK_BN_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (V_TYPE.equals("S")) {
				...
				cs = conn.prepareCall("call DISTR_E_RISK.USP_E_RISK_RT_DISTR(?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (V_TYPE.equals("S")) {
				...
				cs = conn.prepareCall("call DISTR_E_RISK.USP_E_RISK_S_RT_DISTR(?,?,?,?,?,?,?,?,?,?,?,?)");
