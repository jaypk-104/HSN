...
Ext.define('M_CHARGE_STEEL.view.MyGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.myGrid',

    requires: [
        'M_CHARGE_STEEL.view.MyGridViewModel',
        'Ext.view.Table',
    			...
    			listeners : {
    				afterrender : function(c) {
    					c.getEl().on('dblclick', function() {
    						...
    						Ext.getCmp('fieldType').setValue('M_CHARGE_STEEL1');
    			...
    			listeners : {
    				afterrender : function(c) {
    					c.getEl().on('dblclick', function() {
    						...
    						Ext.getCmp('fieldType').setValue('M_CHARGE_STEEL2');
    						var store = Ext.getStore('WinBpPopStore');
    						store.removeAll();
    					});
    				}
            ...
            listeners: {
            	edit: function(editor, context, eOpts) {
            		...
            		if (context.column.dataIndex == 'VAT_TYPE' && VAT_TYPE == 'A') { //부가세유형 - 일반세금계산서
            			...
            			context.record.set('PAY_AMT', LOC_AMT);
            		}
            		else if (context.column.dataIndex == 'VAT_TYPE' && VAT_TYPE == 'C') { //부가세유형 - 영세율
        			...
        			else if (VAT_TYPE == 'I' && context.column.dataIndex == 'LOC_AMT') {
        				context.record.set('VAT_AMT', Math.floor(LOC_AMT * 0.1 / 10) * 10);   //Math.floor(n/10) * 10
        				context.record.set('VAT_RATE', 10);
        				context.record.set('PAY_AMT', LOC_AMT + (Math.floor(LOC_AMT * 0.1 / 10) * 10));
        			}
        			else if (VAT_TYPE == 'I' && context.column.dataIndex == 'VAT_REF_AMT') {
        				context.record.set('VAT_AMT', Math.floor(VAT_REF_AMT * 0.1 / 10) * 10);   //Math.floor(n/10) * 10
        				context.record.set('VAT_RATE', 10);
        				context.record.set('PAY_AMT', VAT_REF_AMT + (Math.floor(VAT_REF_AMT * 0.1 / 10) * 10));
        			}
