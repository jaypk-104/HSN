...
Ext.define('M_GR_TOT_HSPF.view.MyGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.myGrid',

    requires: [
        'M_GR_TOT_HSPF.view.MyGridViewModel',
        'Ext.view.Table',
        ...
        listeners: {
        	select: function(rowmodel, record, index, eOpts) {
    			...
    			store.each(function(rec, index) {
    				if(!!!record.get('CC_NO')){
    					if(!!!rec.get('CC_NO') && record.get('BL_NO') == rec.get('BL_NO')){
    						selModel.select(rec, true);
    					}
    				} else {
    					if(rec.get('CC_NO') && record.get('CC_NO') == rec.get('CC_NO')){
    						selModel.select(rec, true);
    					}
    				}
    			});
            ...
            listeners: {
            	...
            	afteredit : function(e) {
					...
					if(field == 'SL_CD'){
						...
						store.each(function(rec, idx) {
							if(!!!record.get('CC_NO')){
								if(!!!rec.get('CC_NO') && record.get('BL_NO') == rec.get('BL_NO') && (rec.get(field) == null || rec.get(field) == '')){
		    						rec.set(field, val);
		    					}		    					
		    				} else {
		    					if(rec.get('CC_NO') && record.get('CC_NO') == rec.get('CC_NO') && (rec.get(field) == null || rec.get(field) == '')){
		    						rec.set(field, val);
		    					}
		    				}
		            	});
