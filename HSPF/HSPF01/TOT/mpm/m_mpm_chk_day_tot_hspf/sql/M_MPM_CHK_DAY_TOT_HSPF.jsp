...
<%
	...
	try {
		...
		String V_DT_FR = request.getParameter("V_DT_FR") == null ? "" : request.getParameter("V_DT_FR").replace("-", "").substring(0, 8);
		String V_DT_TO = request.getParameter("V_DT_TO") == null ? "" : request.getParameter("V_DT_TO").replace("-", "").substring(0, 8);
		...
		if (V_TYPE.equals("S")) {
			cs = conn.prepareCall("call USP_003_M_MPM_DAY_TOT_HSPF(?,?,?,?,?,?,?,?,?)");
