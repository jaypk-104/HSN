...
<body>
	...
	<div id="prt_div" >
	<iframe
		src="http://123.142.124.170:8080/aireport/on_server/S_DN_PAPER_Y.jsp?PRINT_NO=<%=request.getParameter("PRINT_NO")%>"
...
<%request.setCharacterEncoding("utf-8");
			...
			try {
				...
				cs1 = conn.prepareCall("call USP_PRINT_CNT(?,?)");
				...
				System.out.println(request.getParameter("PRINT_NO"));
			...
			<%
			if (status.equals("N")) { //미승인상태이면, 버튼 보이기
			%>
			document.getElementById("top_div").style.display = "block";
			<%
			} else if (status.equals("Y")) { //승인상태면, 버튼 숨기기
			%>
			document.getElementById("top_div").style.display = "none";
			<%
			} else if (status.equals("X")) { //삭제된 요청서이면, 출력물 숨기기
			%>
			document.getElementById("prt_div").style.display = "none";
			alert('삭제된 요청서입니다.');
			<%
			}
			%>
...
<%ResultSet rs = null;
			...
			try {
				...
				cs = conn.prepareCall("call DISTR_SALES.USP_DN_PRINT_NEW(?,?,?,?,?,?,?,?,?,?,?,?)");
				...
				cs.setString(8, status);//V_BLCN_NO
