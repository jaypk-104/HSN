			...
			if (FileUpload.isMultipartContent(request)) {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField() && item.getSize() > 0) {
						...
						V_FILE_IN_SYSTEM_NM =V_COMP_ID + "_" +  V_ASN_NO + '_' + time + ext;
...
// 						File file = new File("C:\\OmegaPlus\\upload\\default\\AsnUploadFile\\" + V_FILE_IN_SYSTEM_NM);
						File file = new File("F:\\Dev\\fileupload\\" + V_FILE_IN_SYSTEM_NM);

						request.setAttribute("file", file);
						item.write(file);	
						item.delete();

						cs = conn.prepareCall("{call USP_Q_ASN_FILE(?,?,?,?,?,?,?,?)}");
...
// 			String savePath = "C:\\OmegaPlus\\upload\\default\\AsnUploadFile\\" + V_COMP_ID + "\\";
			String savePath = "F:\\Dev\\fileupload\\";
			...
			while (rs.next()) {
				...
				subObject.put("FILE_NM", rs.getString("FILE_NAME"));
		...
		/*파일삭제*/
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					V_COMP_ID = hashMap.get("COMP_ID") == null ? "" : hashMap.get("COMP_ID").toString();
				...
				V_COMP_ID = jsondata.get("COMP_ID") == null ? "" : jsondata.get("COMP_ID").toString();
				...
				// 				System.out.println("V_TYPE2" + V_TYPE);
				// 				System.out.println("V_ASN_NO2" + V_ASN_NO);
				// 				System.out.println("V_FILE_NM2" + V_FILE_NM);
				// 				System.out.println("V_FILE_IN_SYSTEM_NM2" + V_FILE_IN_SYSTEM_NM);
