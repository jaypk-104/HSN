...
<%
	...
	try {
		...
		//관리구분팝업조회
		if (V_TYPE.equals("SP")) {
			cs = conn.prepareCall("call USP_003_M_CHARGE_POPUP_TOT_HSPF(?,?,?,?,?,?,?,?,?)");
			...
			cs = conn.prepareCall("call USP_003_M_CHARGE_TOT_HSPF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
// 					APPROV_NM = rs.getString("APPROV_NM");
				...
				while (rs.next()) {
					...
					subObject.put("TEMP_GL_NO", rs.getString("TEMP_GL_NO"));
					subObject.put("VAT_FG", rs.getString("VAT_FG"));
					subObject.put("BL_LOC_AMT", rs.getString("BL_LOC_AMT"));
					jsonArray.add(subObject);
				}
				jsonObject.put("success", true);
			...
			cs = conn.prepareCall("call USP_003_M_CHARGE_CALC_1(?,?,?,?,?,?,?)");
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					String V_VAT_FG = hashMap.get("VAT_FG") == null ? "" : hashMap.get("VAT_FG").toString();
...
// 					System.out.println("V_TAX_BP_CD : " + V_TAX_BP_CD);
					...
					if (!V_M_BP_CD.equals("")) {
						...
						cs.setString(35, V_USR_ID);//V_USR_ID
						cs.registerOutParameter(36, com.tmax.tibero.TbTypes.CURSOR);
						cs.setString(37, V_VAT_FG);//V_VAT_FG
						cs.executeQuery();
				...
				String V_VAT_FG = jsondata.get("VAT_FG") == null ? "" : jsondata.get("VAT_FG").toString();
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (V_EXP_ITEM_FLG.equals("true") || V_EXP_ITEM_FLG.equals("Y")) {
						...
						cs2 = conn.prepareCall("call USP_003_M_CHARGE_CALC_1(?,?,?,?,?,?,?)");
...
// 			System.out.println(jsonData);
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					if (V_TYPE.equals("I")) {
						...
						cs = conn.prepareCall("call USP_003_TEMP_M_CHARGE_NO(?,?)");
						...
						/*제일 마지막 한번*/
						if (i == jsonAr.size() - 1) {
							cs = conn.prepareCall("call USP_003_A_TEMP_CH_FR_TOT(?,?,?,?,?,?,?,?)");
							cs.setString(1, V_COMP_ID);//V_COMP_ID
				...
				if (V_TYPE.equals("I")) {
					...
					cs.registerOutParameter(6, com.tmax.tibero.TbTypes.CURSOR);
					cs.setString(7, V_CHARGE_CD);//V_CHARGE_CD
					cs.setString(8, V_TEMP_GL_NO);//V_TEMP_GL_NO
					cs.executeQuery();
			...
			if (V_TEMP_GL_NO.contains("TG")) {
				/*애니링크 시작*/
				System.out.println("애니링크 시작 V_TYPE : " + V_TYPE);
