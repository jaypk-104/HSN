	...
	try {
		...
		if (V_TYPE.equals("SP")) {
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					HashMap hashMap = (HashMap) jsonAr.get(i);
					String CHARGE_CD = hashMap.get("CHARGE_CD") == null ? "" : hashMap.get("CHARGE_CD").toString();
					V_TYPE = hashMap.get("V_TYPE") == null ? "" : hashMap.get("V_TYPE").toString();

					//V_TYPE이 공백이 아닐때만 수행.
					if (!V_TYPE.equals("")) {
						if (CHARGE_CD.equals("SPC41")) { /*인수수수료 ONLY*/

							cs = conn.prepareCall("call DISTR_TEMP_GL.USP_A_TEMP_REC_FR(?,?,?,?,?)");
							cs.setString(1, V_COMP_ID);//V_COMP_ID
							...
							if (flag.equals("Y") && V_PR_STEP.equals("VB")) { //통합전표 생성을 1회 수행하였을 경우 for문 continue;
								continue;
							} else {
								/*통합전표*/
								cs = conn.prepareCall("call DISTR_TEMP_GL.USP_A_TEMP_CH_FR(?,?,?,?,?,?)");
								cs.setString(1, V_COMP_ID);//V_COMP_ID
								cs.setString(2, V_TYPE);//V_TYPE
								cs.setString(3, V_MAST_CHARGE_NO);//V_MAST_CHARGE_NO
								cs.setString(4, V_USR_ID);//V_BL_SEQ
								cs.registerOutParameter(5, com.tmax.tibero.TbTypes.CURSOR);
								cs.setString(6, CHARGE_CD);//V_CHARGE_CD
								cs.executeQuery();
				...
				JSONObject jsondata = JSONObject.fromObject(jsonData);
				String CHARGE_CD = jsondata.get("CHARGE_CD") == null ? "" : jsondata.get("CHARGE_CD").toString();
				V_TYPE = jsondata.get("V_TYPE") == null ? "" : jsondata.get("V_TYPE").toString();

				// 				System.out.println("zzz" + V_TYPE);
				if (!V_TYPE.equals("")) {
					if (CHARGE_CD.equals("SPC41")) { /*인수수수료 ONLY*/
						...
						cs.setString(4, V_USR_ID);//V_BL_SEQ
						cs.registerOutParameter(5, com.tmax.tibero.TbTypes.CURSOR);
						cs.setString(6, CHARGE_CD);//CHARGE_CD
						cs.executeQuery();
