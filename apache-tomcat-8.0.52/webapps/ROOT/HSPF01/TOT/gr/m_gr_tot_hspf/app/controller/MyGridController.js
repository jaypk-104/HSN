...
Ext.define('M_GR_TOT_HSPF.controller.MyGridController', {
...
//		}
		...
		if(Ext.getCmp('V_GR_SL_CD').getValue() == '' || Ext.getCmp('V_GR_SL_CD').getValue() == null || Ext.getCmp('V_GR_SL_CD').getValue() == undefined ){
			flag = 'F';
			msg = '입고창고를 입력해주세요.';
		}
		else{
		...
		for (var i = 0; i < gridRecord.length; i++) {
			if (gridRecord[i].data['REQ_QTY'] > 0) {
				...
				if(gridRecord[i].data['GR_NO'] != null && gridRecord[i].data['GR_NO'] != undefined && gridRecord[i].data['GR_NO'] != ''){
					ext_msg +=  '<br><br>' + gridRecord[i].data['CC_NO'] + ' 는 동일한 통관관리번호로 먼저 입고된 데이터가 존재합니다.<br>먼저 입고된 데이터의 입고일로 저장됩니다.'
				}
		...
		if(gridRecord.length != GR_COUNT) {
			Ext.Msg.confirm('확인', '등록하시겠습니까?<br>입고일 : ' + GR_DT + ext_msg, function(button) {
				if (button == 'yes') {
					var gridRecord = Ext.getCmp('myGrid').getSelectionModel().getSelection();

					Ext.Ajax.request({
						url : 'sql/M_GR_TOT_HSPF_H.jsp',
						method : 'post',
						params : {
							V_TYPE : 'IH',
...
//			CC_NO += ccArr[i] +',';
//		}
//		CC_NO = CC_NO.substr(0, CC_NO.length -1);
