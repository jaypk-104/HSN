...
// 		System.out.println("V_PO_NO" + V_PO_NO);
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call DISTR_GR.USP_M_GR_KOR_REF_DISTR(?,?,?,?,?,?,?,?)");
			...
			sql += "       LEFT OUTER JOIN M_PO_HDR_DISTR H                                                  ";
			sql += "       ON     A.COMP_ID=H.COMP_ID                                                        ";
			sql += "       AND    A.PO_NO  =H.PO_NO                                                          ";
			sql += "       LEFT OUTER JOIN B_ITEM_BF_DISTR B                                                 ";
			...
			sql += "       LEFT OUTER JOIN B_STORAGE_DISTR G                                                 ";
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					cs = conn.prepareCall("call DISTR_GR.USP_M_GR_KOR_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					cs.setString(1, V_COMP_ID);//V_COMP_ID
					...
					cs.setString(20, BOX_QTY);//BOX_QTY
					cs.setString(21, BOX_WGT_UNIT);//BOX_WGT_UNIT
...
// 			System.out.println("V_MVMT_DT" + V_MVMT_DT);
