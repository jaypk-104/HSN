...
<%
	...
	try {
		...
		if (V_TAB_TYPE.equals("T1") && V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call DISTR_TR.USP_I_SL_MV_H_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			cs.setString(23, V_BL_DOC_NO);//V_BL_DOC_NO
			cs.executeQuery();
		...
		else if (V_TAB_TYPE.equals("T1") && V_TYPE.equals("ERP")) {
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int k = 0; k < grArr_FIN.size(); k++) {
					cs = conn.prepareCall("call DISTR_TEMP_GL.USP_A_TEMP_SL_TR(?,?,?,?,?)");

					cs.setString(1, V_COMP_ID);//V_COMP_ID
					cs.setString(2, U_TYPE);//V_TYPE
					cs.setString(3, grArr_FIN.get(k));//
		...
		/*항운노조비등록*/
			...
			cs = conn.prepareCall("call DISTR_TR.USP_I_PORT_H_DISTR(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_TYPE);//V_TYPE
			cs.setString(2, V_COMP_ID);//V_COMP_ID
