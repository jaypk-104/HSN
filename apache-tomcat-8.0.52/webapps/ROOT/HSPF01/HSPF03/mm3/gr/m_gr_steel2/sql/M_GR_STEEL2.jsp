...
// 		System.out.println("V_TYPE : " + V_TYPE);
		...
		if (V_TYPE.equals("SH")) {
			...
			cs = conn.prepareCall("call USP_001_M_GR_KOR_REF_STEEL(?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
			cs.setString(2, V_PO_NO);//V_ID_DT_FR
			cs.setString(3, V_APPROV_NO);//V_ID_DT_TO
			cs.setString(4, V_ITEM_CD);//V_BL_DOC_NO
			cs.setString(5, V_PO_DT_FR);//V_M_BP_NM
			cs.setString(6, V_PO_DT_TO);//V_SL_CD
			cs.registerOutParameter(7, com.tmax.tibero.TbTypes.CURSOR);
			cs.setString(8, V_M_BP_NM);//V_SL_CD
			cs.executeQuery();
...
// 				subObject.put("DATA", rs.getString("DATA"));
			...
			sql += "       F.DTL_NM PAY_METH_NM, A.ITEM_CD, B.ITEM_NM, '' SPEC, A.BOX_QTY, A.BOX_WGT_UNIT,   ";
			sql += "       A.QTY, A.CUR, A.SL_CD, G.SL_NM, A.DOC_AMT, A.LOC_AMT, TO_CHAR(A.MVMT_DT, 'YYYY-MM-DD') MVMT_DT                              ";
			sql += "FROM   M_GR_STEEL A                                                                      ";
			sql += "       LEFT OUTER JOIN M_PO_HDR_STEEL H                                                  ";
			sql += "       ON     A.COMP_ID=H.COMP_ID                                                        ";
			sql += "       AND    A.PO_NO  =H.PO_NO                                                          ";
			sql += "       LEFT OUTER JOIN B_ITEM_BF_STEEL B                                                 ";
			...
			sql += "       LEFT OUTER JOIN B_STORAGE_STEEL G                                                 ";
			...
			*/
			...
			cs = conn.prepareCall("call USP_001_M_GR_KOR_MID_STEEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			cs.setString(1, V_COMP_ID);//V_COMP_ID
			cs.setString(2, V_TYPE);//V_TYPE
			cs.setString(3, V_MVMT_NO);//MVMT_NO
			cs.setString(4, "");//FIRST_YN
			cs.setString(5, "");//ITYPE
			cs.setString(6, "");//MVMT_DT
			...
			if (jsonData.lastIndexOf("},{") > 0) { //배열일경우
				...
				for (int i = 0; i < jsonAr.size(); i++) {
					...
					cs.setString(19, "");//FORWARD_XCH_AMT
					cs.setString(20, BON_QTY);//BOX_QTY
					cs.setString(21, BON_WGT_UNIT);//BOX_WGT_UNIT
					cs.setString(22, "");//DN_QTY
					cs.setString(23, SL_CD);//SL_CD
					cs.setString(24, BL_DOC_NO);//BL_DOC_NO
					cs.setString(25, "");//CC_SEQ
					cs.setString(26, V_USR_ID);//USR_ID
					cs.registerOutParameter(27, com.tmax.tibero.TbTypes.CURSOR);
					cs.setString(28, DATA);//DATA
					cs.setString(29, PLANT_NO);//DATA
					cs.executeQuery();
...
// 				JSONObject jsondata = JSONObject.fromObject(jsonData);
				...
				cs.registerOutParameter(27, com.tmax.tibero.TbTypes.CURSOR);
				cs.setString(28, DATA);//DATA
				cs.setString(29, PLANT_NO);//PLANT_NO
				cs.executeQuery();
